...
### 3.7 渲染层测试分工策略（2026-08-31 制定）

渲染层规范目标为 80/75/80/80，但**分层目标不等于全员单测**——纯展示组件由
E2E 兑底是正式策略而非欠账：

| 组件类别 | 测试方式 | 理由 |
|---|---|---|
| 逻辑密度高的模块（transport/hook/纯函数/交互组件） | 单测全覆盖分支 | 单测 ROI 最高，可锚定失败路径 |
| 巨型集成容器（ChatPanel 等） | 职责拆分为可测子模块 + E2E journey 兑底 | 单测 mock 面过大，伪测试风险高 |
| 纯展示组件（message-item/Markdown/settings sections） | E2E journey + visual.spec 像素快照 | 单测只能断言 DOM 存在，价值低 |

- 数字真源：scripts/coverage-floors.json（floor/ratchet 只升不降，实测与目标差
  逐层打印）；**不得对外表述为"渲染层已达标 80/75/80/80"**。
- 缺口定性：settings 组件约 21%、Markdown/message-item 等属**策略性不补**，
...
> - shared：80/30/39/80（当前 89.06/44.44/44.82/88.95，2026-08-27 全链审计按棘轮机制上调）
> - main：80/75/80/80（当前 92.45/84.59/89.57/92.45，2026-08-12 提升）
> - renderer：59/50/54/59（当前 64.06/55.91/59.96/64.87，2026-08-27 全链审计纠正：此前记录 92.87 与实测不符，已按实测−5 重设基线；补测后按机制上调）
