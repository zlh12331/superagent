...
需要某种能力（解析、校验、度量、格式化…）时，**先用现成的成熟库**，只在没有合适依赖时才自己写。
本项目已有前车之鉴：函数度量曾用自研正则实现，在一个函数内就暴露 3 个 bug
（`for` 头的分号清空状态、`&&` 链被操作数打断、`else if` 重复计分），且语义假设与权威实现
（Biome）不符——改用现成规则后问题整体消失。**社区的边界情况远比自研考虑得全。**

> ⚠️ 例外是**平台硬约束**，不适用本原则：`preload` 必须 CJS 且零运行时依赖
> （Electron 沙箱限制，zod 进 preload 会静默失败）、`packages/shared/src/ipc/meta.ts`
> 必须纯字符串。这类是「技术上做不到」，不是「不愿加依赖」。
