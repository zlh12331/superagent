...
import type { IRemoteControlService, RemoteCommand } from './remote-control';
...
function createStubs(options: { history?: readonly unknown[] } = {}) {
  const commandHandlers: Array<
    (command: RemoteCommand) => Promise<{ accepted: boolean; reply?: string }>
  > = [];
  ...
  const mockOnCommand = vi.fn(
    (handler: (command: RemoteCommand) => Promise<{ accepted: boolean; reply?: string }>) => {
...
function emit(stubs: Stubs, event: EmitEvent): void {
...
async function runCommand(
  stubs: Stubs,
  text: string,
  options: { assistantText?: string; reason?: string } = {},
  ...
  const pending = handler(command({ text }));
  await vi.waitFor(() => expect(stubs.turnListeners.length).toBeGreaterThan(0));
  emit(stubs, { type: TurnEventType.TURN_START, sessionId: 'session-1', turnId: 'turn-1' });
...
describe('RemoteAgentBridge', () => {
  ...
  it('auto 模式：无头执行回合并回传正文（sessionId 取 create 返回值）', async () => {
    ...
    expect(result).toEqual({ accepted: true, reply: '34 个文件全部通过' });
  ...
  it('ask 模式：拒绝执行并回传可读提示（不启动 agent）', async () => {
    ...
    const [handler] = stubs.commandHandlers;
    const result = await handler?.(command({ text: '危险操作' }));
  ...
  it('同一 clientId 串行：执行中的新命令回传排队提示', async () => {
    ...
    const [handler] = stubs.commandHandlers;
    if (handler === undefined) throw new Error('桥接未挂载 onCommand');
    const first = handler(command({ text: '长任务' }));
    await vi.waitFor(() => expect(stubs.turnListeners.length).toBeGreaterThan(0));

    const busy = await handler(command({ text: '插队' }));
    expect(busy).toEqual({ accepted: true, reply: expect.stringContaining('上一条命令仍在执行') });

    emit(stubs, { type: TurnEventType.TURN_END, sessionId: 'session-1', reason: 'completed' });
    await first;
    // 首回合结束后同一客户端可继续执行
    const third = handler(command({ text: '后续' }));
    await vi.waitFor(() => expect(stubs.turnListeners.length).toBeGreaterThan(0));
    emit(stubs, { type: TurnEventType.TURN_END, sessionId: 'session-1', reason: 'completed' });
  ...
  it('他人会话事件不串台：仅本 session 的 TURN_END 结束本回合', async () => {
    ...
    const [handler] = stubs.commandHandlers;
    if (handler === undefined) throw new Error('桥接未挂载 onCommand');
    const pending = handler(command({ text: '任务' }));
    await vi.waitFor(() => expect(stubs.turnListeners.length).toBeGreaterThan(0));

    // 桌面端并发会话的结束事件 + 文本增量必须被丢弃
    emit(stubs, { type: TurnEventType.TEXT_DELTA, sessionId: 'other-session', text: '串台内容' });
    emit(stubs, { type: TurnEventType.TURN_END, sessionId: 'other-session', reason: 'completed' });
    emit(stubs, { type: TurnEventType.TEXT_DELTA, sessionId: 'session-1', text: '正确内容' });
    emit(stubs, { type: TurnEventType.TURN_END, sessionId: 'session-1', reason: 'completed' });

    expect(await pending).toEqual({ accepted: true, reply: '正确内容' });
  });
  ...
  it('非 completed 结束：回传带状态标注；工具事件计入摘要', async () => {
    ...
    await vi.waitFor(() => expect(stubs.turnListeners.length).toBeGreaterThan(0));
    emit(stubs, { type: TurnEventType.TURN_START, sessionId: 'session-1', turnId: 'turn-1' });
    emit(stubs, { type: TurnEventType.TOOL_CALL, sessionId: 'session-1', toolName: 'bash' });
    emit(stubs, { type: TurnEventType.TURN_END, sessionId: 'session-1', reason: 'max-steps' });
  ...
  it('落库失败静默：仍回传执行结果', async () => {
    ...
    const pending = handler(command({ text: '任务' }));
    await vi.waitFor(() => expect(stubs.turnListeners.length).toBeGreaterThan(0));
    emit(stubs, { type: TurnEventType.TEXT_DELTA, sessionId: 'session-1', text: '结果' });
    emit(stubs, { type: TurnEventType.TURN_END, sessionId: 'session-1', reason: 'completed' });
    const result = await pending;
  ...
  it('历史回读失败：降级为单轮执行（不拒绝命令）', async () => {
    ...
    expect(result).toEqual({ accepted: true, reply: '仍然可用' });
    const call = stubs.startAgentCalls[0];
  ...
  it('startAgent 抛错：回传失败提示（不抛给传输层）', async () => {
    ...
    const [handler] = stubs.commandHandlers;
    const result = await handler?.(command({ text: '任务' }));
