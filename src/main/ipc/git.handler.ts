...
// Git 域 IPC handler（GitService 暴露给渲染层的入口，定义表驱动）
//
// 实现 5 个请求-响应方法：
// 只读：
// - status  获取工作区状态（branch/ahead/behind/files/clean）
// - diff    获取 diff（unstaged / staged / 对比任意 ref）
// 写操作：
// - add     暂存工作区改动（git add）
...
//
// 设计要点：
// - DI 模式：通过 deps 注入 IGitService 实例
...
export function createGitHandlers(
  deps: GitHandlerDeps,
): InferHandlers<typeof IPC_DEFINITIONS, IpcHandlerContext>['git'] {
