//! Hook 命令式输入/输出的 JSON Schema 定义与 fixture 生成。
//!
//! 该模块定义了与 Claude hook 命令行协议兼容的输入/输出 wire 类型，
//! 同时为它们生成 JSON Schema fixture 文件，供外部工具参考。
//! Codex 在协议基础上扩展了 `turn_id` 字段，以支持内部 turn 范围的 hook。
...
/// 一个允许为 `null` 的字符串包装类型，序列化为透明字符串或 null。
...
/// 子代理命令输入字段集合，仅当 hook 在子代理内运行时填充。
...
/// 钩子事件名称的 wire 表示，与 JSON 中出现的字符串保持一致。
...
/// PermissionRequest 钩子命令的输出 wire 类型。
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
#[serde(deny_unknown_fields)]
...
pub(crate) struct PermissionRequestDecisionWire {
    ...
    /// 预留给未来的输入重写能力。
    ///
    /// 当前 PermissionRequest hook 在出现此字段时会以失败方式关闭。
    #[serde(default)]
    ...
    /// 预留给未来的短路语义。
    ///
    /// 当前 PermissionRequest hook 在此字段为 `true` 时会以失败方式关闭。
    #[serde(default)]
    pub interrupt: bool,
}

/// PermissionRequest 的行为枚举。
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
...
/// PermissionRequest 钩子命令的输入 wire 类型。
#[derive(Debug, Clone, Serialize, JsonSchema)]
#[serde(deny_unknown_fields)]
...
/// UserPromptSubmit 钩子命令的输出 wire 类型。
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
#[serde(deny_unknown_fields)]
...
pub(crate) struct StopCommandOutputWire {
    ...
    #[serde(default)]
    pub decision: Option<BlockDecisionWire>,
    /// Claude 要求 `decision` 为 `block` 时必须提供 `reason`；
    /// 该语义约束在输出解析阶段而非 JSON Schema 中强制执行。
    #[serde(default)]
    pub reason: Option<String>,
}

/// SubagentStop 钩子命令的输出 wire 类型。
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
#[serde(deny_unknown_fields)]
...
impl SessionStartCommandInput {
    /// 便捷构造函数，自动填充 `hook_event_name` 为 `"SessionStart"`。
    pub(crate) fn new(
...
/// UserPromptSubmit 钩子命令的输入 wire 类型。
#[derive(Debug, Clone, Serialize, JsonSchema)]
#[serde(deny_unknown_fields)]
...
/// 将所有 hook 命令的输入/输出 JSON Schema 写入指定根目录下的 `generated` 子目录。
///
/// 该函数会先清空已存在的 `generated` 目录，再重新生成全部 fixture。
...
/// 规范化 JSON：递归按 key 排序对象字段，使输出可稳定比较。
