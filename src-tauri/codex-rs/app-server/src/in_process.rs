//! 供本地 embedder 使用的 in-process app-server runtime host。
//!
//! 本模块在 Tokio task 上运行现有的 [`MessageProcessor`] 和 outbound routing logic，
//! 但用有界的 in-memory channel 替换 socket/stdio transport。其目的是保持 app-server
//! 语义，同时避免为在同一 process 中运行的 CLI surface 引入 process boundary。
//!
//! # 生命周期
//!
//! 1. 使用 [`InProcessStartArgs`] 构造 runtime 状态。
//! 2. 调用 [`start`]，它会在内部执行 `initialize` / `initialized` 握手，
//!    并返回一个 ready-to-use 的 [`InProcessClientHandle`]。
//! 3. 通过 [`InProcessClientHandle::request`] 发送 request，通过
//!    [`InProcessClientHandle::notify`] 发送 notification，并通过
//!    [`InProcessClientHandle::next_event`] 消费 event。
//! 4. 使用 [`InProcessClientHandle::shutdown`] 终止。
//!
//! # Transport 模型
//!
//! 该 runtime 是 transport-local 的，但并非 protocol-free。Incoming request 是
//! 有类型的 [`ClientRequest`] 值，但 response 仍通过 `MessageProcessor` 用于
//! stdio 和 websocket transport 的相同 JSON-RPC result envelope 返回。这使得
//! in-process 行为与 app-server 保持一致，而非创建第二套执行契约。
//!
//! # Backpressure
//!
//! Command 提交使用 `try_send`，可能返回 `WouldBlock`；而 event fanout 在饱和时
//! 可能丢弃 notification。Server request 绝不会被静默放弃：如果无法入队，它们会
//! 以 overload 或 internal error 失败回 `MessageProcessor`，以确保 approval flow
//! 不会无限挂起。
//!
//! # 与 `codex-app-server-client` 的关系
//!
//! 本模块提供低层 runtime handle（[`InProcessClientHandle`]）。
//! 高层调用者（TUI、exec）应通过 `codex-app-server-client` 使用，后者在本模块
//! 之上封装了 worker task，提供 async request/response helper、surface 专属的
//! startup policy 以及有界 shutdown。
...
/// 供 in-process client 调用 app-server 并消费 event 的 handle。
///
/// 这是低层 runtime handle。高层调用者通常应通过 `codex-app-server-client` 使用，
/// 后者增加了 worker-task buffering、request/response helper 以及 surface 专属的
/// startup policy。
...
impl InProcessClientHandle {
    /// 向 in-process runtime 发送一个有类型的 client request。
    ///
    /// 返回值是 transport 层的 `IoResult`，包含 JSON-RPC success payload 或
    /// JSON-RPC error payload。调用者必须在并发 request 中保持 request id 唯一；
    /// 复用正在使用中的 id 会产生 `INVALID_REQUEST` response，并可能导致调用者
    /// 的 request 路由产生歧义。
    pub async fn request(&self, request: ClientRequest) -> IoResult<PendingClientRequestResponse> {
    ...
    ///
    /// 仅应使用从当前 runtime event stream 接收到的 request id；
    /// 发送任意 id 对 app-server 状态没有影响，且可能掩盖调用者中
    /// 卡住的 approval flow。
    pub fn respond_to_server_request(&self, request_id: RequestId, result: Result) -> IoResult<()> {
...
/// 启动一个 in-process app-server runtime 并执行 initialize 握手。
///
/// 该函数在返回 handle 之前会先发送 `initialize`，紧接着发送 `initialized`，
/// 因此调用者拿到的是一个 ready-to-use 的 runtime。如果 initialize 失败，
/// runtime 会被 shutdown 并返回 `InvalidData` 错误。
