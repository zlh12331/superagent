...
/// 尝试从已 loaded thread 满足 resume 请求的结果。
enum RunningThreadResumeResult {
    /// 该 request 已被 delegated 给 loaded thread。
    Handled,
    /// 没有 loaded thread 处理该 request。
    ///
    /// 可选的 stored thread 包含 history-bearing probe，cold resume
    /// 可以复用它而无需再次读取 rollout。
...
impl ThreadRequestProcessor {
    ...
    async fn thread_start_task(
        ...
        // user 可能通过命令行请求了 WorkspaceWrite 或 DangerFullAccess，
        // 但在派生 Config 的过程中，它可能被降级为 ReadOnly（例如 Windows 上
        // 没有 sandbox 可用，或企业配置不允许）。在这种情况下，cwd 仍应
        // 被视为"trusted"。
            ...
            .await
        {
            Ok(NewThread {
                ...
                if let Some(token_usage_thread) = token_usage_thread {
                    ...
                    // 客户端在开始下一个 turn 之前需要 restored usage。
                    // 在 response 之后发送可以保持 JSON-RPC request 的顺序，
                    // 同时仍能在下一个 turn 生命周期开始前填充状态行。
