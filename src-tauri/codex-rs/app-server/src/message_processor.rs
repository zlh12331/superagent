...
impl MessageProcessor {
    /// 创建一个新的 `MessageProcessor`，并保留 outgoing `Sender` 的 handle，
    /// 以便 handler 可以将消息入队并写入 stdout。
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        // thread store 刻意设计为 process 范围。Config reload 可能影响
        // 单个 thread 的行为，但绝不能将新启动、resumed 或 forked 的 thread
        // 迁移到不同的 persistence backend/root。
