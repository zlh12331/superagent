...
// macOS arm64 与 Windows Bazel CI 环境下，app-server 启动可能需要数十秒，
// initialize 响应或 fuzzy-search 通知才能到达，因此需要更长的超时时间。
...
async fn test_fuzzy_file_search_sorts_and_includes_indices() -> Result<()> {
    // 准备一个临时的 Codex home，以及一个独立、包含测试文件的根目录。
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path())?;
    let root = TempDir::new()?;

    // 创建若干文件，使得对查询字符串 "abe" 的排序结果具有确定性（便于断言）。
