...
async fn review_start_runs_review_turn_and_emits_code_review_item() -> Result<()> {
    ...
    // 确认我们在 main thread 上能观察到 EnteredReviewMode marker。
    // 断言意图：循环读取 item/started 通知，命中 EnteredReviewMode 即标记成功，
    // 验证 review 模式进入事件被正确投递到当前 thread 的事件流。
    ...
    // 确认我们在同一个 turn 上能观察到 ExitedReviewMode marker（携带 review 文本）。
    // 流中可能出现其他 item，这里忽略它们，只关注退出 review 模式的事件。
