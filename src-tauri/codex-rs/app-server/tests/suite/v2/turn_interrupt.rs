...
async fn turn_interrupt_aborts_running_turn() -> Result<()> {
    // 使用可移植的 sleep 命令保持 turn 处于运行状态，以便后续打断。
    ...
    // Mock server：先返回一个长时间运行的 shell 命令；turn 被打断后无需再返回内容。
    let server =
    ...
    // 启动一个 turn，触发一个长时间运行的命令，便于后续验证打断行为。
    let turn_req = mcp
    ...
    // 等待片刻，确保命令已经真正开始执行，再发起打断。
    tokio::time::sleep(std::time::Duration::from_secs(1)).await;

    let thread_id = thread.id.clone();
    // 通过 turn id 调用 v2 API 打断正在进行的 turn。
