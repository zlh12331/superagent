...
async fn thread_start_creates_thread_and_emits_started() -> Result<()> {
    ...
    // 启动一个 v2 thread，并通过参数显式覆盖默认 model。
    let req_id = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
    ...
    // 期望返回一个合法的 JSON-RPC 响应，且其中包含 thread id。
    ...
    // Wire 契约：thread 的标题字段为 `name`，未设置时序列化为 null。
...
async fn thread_start_drops_unsupported_service_tier_id() -> Result<()> {
    ...
    // 不支持的 catalog id 在 session 配置阶段会被丢弃，而不会回传给客户端。
