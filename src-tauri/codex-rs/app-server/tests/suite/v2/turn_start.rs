...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // 两次 Codex turn 会命中 mock model（session 启动 + turn/start）。
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    // 这里使用 unchecked 变体，因为严格匹配器目前还不支持携带 image 的请求体。
...
async fn thread_start_omits_empty_instruction_overrides_from_model_request() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            // TODO(aibrahim)：将“空字符串作为 instructions 覆盖”的写法替换为显式的三态
            // app-server 语义：未提供、显式 none、显式具体值。
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    // 提供一个 mock server 和配置，确保模型连线（model wiring）合法可用。
    // 三个 Codex turn 会命中 mock model（session 启动 + 两次 turn/start 调用）。
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    // 启动一个 v2 thread，并捕获其 id，供后续 turn/start 使用。
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            model: Some("mock-model".to_string()),
            ..Default::default()
    ...
    // 启动一个 turn，仅设置 input 与 thread_id（不传入任何 override 参数）。
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
            client_user_message_id: None,
    ...
    // 发起第二个 turn，以覆盖（override）路径为例：修改 model 字段。
    let turn_req2 = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
            client_user_message_id: None,
    ...
    // 确保第二个 turn 的 id 与第一个不同（每个 turn 应有唯一 id）。
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    // Mock server 行为：第一个 turn 请求 shell 调用（触发 elicitation），随后完成；
    // 第二个 turn 同样发起 shell 调用，但我们会把 approval_policy 设为 never 以
    // 跳过 elicitation 流程，用于对比两种策略下的服务端行为。
    let responses = vec![
    ...
    let server = create_mock_responses_server_sequence(responses).await;
    // 默认审批策略设为 untrusted，以强制在首个 turn 触发 elicitation 询问流程。
    create_config_toml(
        codex_home.as_path(),
        &server.uri(),
        "untrusted",
    ...
    // 第二个 turn 设置 approval_policy=never，应不再触发审批 elicitation
    ...
    // 确保任务完成前不再收到 CommandExecutionRequestApproval 请求（避免重复审批）。
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("turn/completed"),
    )
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    // 第一个 turn：使用 workspace-write sandbox 与 first_cwd。
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            environments: None,
            thread_id: thread.id.clone(),
    ...
    // 第二个 turn：使用 workspace-write 与 second_cwd，确保 exec 在 second_cwd 中启动。
    let second_turn = mcp
        .send_turn_start_request(TurnStartParams {
            environments: None,
            thread_id: thread.id.clone(),
...
async fn turn_start_emits_spawn_agent_item_with_model_metadata_v2() -> Result<()> {
    ...
    // 复用当前已启动的进程环境，覆盖 thread/delete 中 ThreadManager 的后代遍历路径。
    let delete_req = mcp
...
"#;
    ...
    // 第一个 turn：预期收到 FileChangeRequestApproval 请求，响应 AcceptForSession，并验证文件确实生成。
    let turn_1_req = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
            client_user_message_id: None,
    ...
    // 第二个 turn：对同一文件再应用一次 patch。由于上一次已 AcceptForSession，
    // 本次应跳过审批。
    let turn_2_req = mcp
        .send_turn_start_request(TurnStartParams {
            thread_id: thread.id.clone(),
    ...
    // 如果服务端错误地再次发出 FileChangeRequestApproval，下面的读取逻辑会
    // 因遇到非预期 JSONRPCMessage::Request 而报错，从而让测试失败。
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("item/completed"),
    )
...
// 辅助函数：创建指向 mock model server 的 config.toml 配置文件。
