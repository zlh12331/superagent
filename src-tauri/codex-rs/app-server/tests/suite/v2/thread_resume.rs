...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    // 启动一个 thread。
    // 调用时机：本用例验证“未物化（unmaterialized）的 thread 不能 resume”，
    // 因此需要在 resume 之前先 start 一个 thread 但不让其落地 rollout。
    let start_id = mcp
    ...
    // 在第一条用户消息落地为 rollout 存储之前，resume 应当失败。
    let resume_id = mcp
...
async fn thread_resume_rejoins_running_thread_even_with_override_mismatch() -> Result<()> {
    ...
    // 运行中 thread 的 resume 响应会被排入 thread listener 任务队列。如果
    // 正在进行的 turn 在该排队命令执行之前就已完成，那么响应中观察到 thread
    // 处于 idle 状态也是合法的。
    match &thread.status {
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    // 使用显式 history 进行 resume，并覆盖 model 配置。
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            thread_id,
