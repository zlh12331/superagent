#![cfg(not(windows))]
//
// 使用打了补丁的 zsh fork 运行这些测试的说明：
//
// 测试套件在首次使用时通过 DotSlash 解析共享的、仅用于测试的 zsh DotSlash 文件
// （位于 `app-server/tests/suite/zsh`），因此首次拉取该构件时需要本地存在
// `dotslash` 工具以及可用的网络访问。
...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    // 让 shell 命令保持运行状态直到我们主动中断它。如果使用 `echo hi` 这类
    // 快速命令，在性能较强的运行机上它可能在中断信号到达之前就已结束，那样
    // 本测试会退化为“命令结束后的后续行为”测试，而非打断一个正在执行的
    // zsh-fork 命令——后者才是测试的本意。
    ...
    // 在 shell item 启动之后再发起中断，可能与上报“被中止工具调用”的后续
    // model 请求产生竞态。本测试只关心 zsh-fork 是否启动了预期命令，因此
    // 允许额外出现一次 no-op 的 `/responses` POST，而不强校验请求总数。
    let server =
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    // Linux CI 偶尔会在子命令拒绝流程之后再发一次 `/responses` POST。本测试
    // 关注的是 zsh fork 中的审批/拒绝行为，而非精确的 model 请求计数，因此
    // 允许出现一次额外请求，并在其到来时返回无害的 no-op 响应。
    let server =
        create_mock_responses_server_sequence_unchecked(vec![response, no_op_response]).await;
    create_config_toml(
        &codex_home,
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            // 本测试关注的是 execve 拦截审批的传播行为，而非 workspace sandbox
            // 隔离。使用 DangerFullAccess 可以避免 macOS sandbox 初始化失败，
            // 这种失败会在第二个子命令审批被观察到之前就终止父 shell。
            sandbox_policy: Some(codex_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
            effort: Some(codex_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(codex_protocol::config_types::ReasoningSummary::Auto),
    ...
    while target_decision_index < target_decisions.len() || !saw_parent_approval {
        ...
        let decision = if is_target_subcommand {
            ...
            // login shell 在执行父 shell 命令或目标子命令之前，可能先运行
            // 一些启动辅助程序（例如 macOS 上的 path_helper），这些也需要放行。
            CommandExecutionApprovalDecision::Accept
        };
        mcp.send_response(
            request_id,
    ...
    match parent_completed_command_execution {
        ...
        Ok(Err(error)) => return Err(error),
        Err(_) => {
            // 部分 zsh 构建在子命令被拒绝后会立即中止 turn，且不会发出父级
            // `item/completed` 通知；此外 Linux 上的 sandbox 失败也可能在
            // 父级完成 item 被观察到之前就让 turn 结束。这里两种情况都按
            // turn/completed 兜底处理。
            let completed_notif = timeout(
                DEFAULT_READ_TIMEOUT,
                mcp.read_stream_until_notification_message("turn/completed"),
