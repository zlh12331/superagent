...
async fn thread_fork_creates_new_thread_and_emits_started() -> Result<()> {
    ...
    // Wire contract 约定：thread 的标题字段为 `name`，未设置时序列化为 null。
    // 断言意图：确保 fork 后的 thread 对象在 JSON 中保持与 wire contract 一致的字段名与空值表示。
...
// 辅助函数：创建一个 config.toml，将其指向 mock model server。
// 调用时机：在每个用例启动 app-server 之前准备 codex_home 配置。
