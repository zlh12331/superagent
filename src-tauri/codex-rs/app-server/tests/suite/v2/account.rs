...
// 辅助函数：为 app server 创建最小化的 config.toml 配置。
#[derive(Default)]
...
#[tokio::test]
// Client 在 refresh 时返回 JSON-RPC 错误；turn 应当失败。
...
#[tokio::test]
// refresh 返回的 token 属于错误的 workspace，turn 应失败。
...
#[tokio::test]
// Refresh 返回格式非法的 access token；turn 应当失败。
...
#[tokio::test]
// 由于 login server 绑定固定端口，相关测试需要串行执行以避免端口冲突。
#[serial(login_port)]
...
async fn set_auth_token_cancels_active_chatgpt_login() -> Result<()> {
    ...
    // 通过设置 external auth token 来代替完成 ChatGPT 登录流程。
    // 这样做应当取消正在进行的登录尝试。
    let set_id = mcp
        .send_chatgpt_auth_tokens_login_request(
            access_token,
    ...
    // 验证之前的登录尝试确实已被取消。
    // 这里通过再次尝试取消它，并期望收到 not found 错误来确认。
    let cancel_id = mcp
        .send_cancel_login_account_request(CancelLoginAccountParams {
            login_id: login_id.clone(),
        })
