...
/// 该 cache 会被视为 fresh（仍在 TTL 有效期内），优先使用本地 cache 而不是从网络重新拉取。
...
pub fn write_models_cache(codex_home: &Path) -> std::io::Result<()> {
    // 取得稳定的 bundled-catalog 派生 preset 列表，并过滤出 picker 可见的条目。
    // 设计理由：使用固定的内置 catalog 而非动态网络数据，保证测试可复现；
    // 仅保留 show_in_picker 为 true 的 preset，与 picker 实际展示口径一致。
