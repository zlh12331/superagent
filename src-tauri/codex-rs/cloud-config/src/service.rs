//! 云端配置 bundle 的生命周期编排。
//!
//! 启动时从缓存或后端加载单个共享 bundle；后台刷新任务会持续更新缓存，
//! 以便后续应用启动可以直接命中缓存，但不会改动已经快照到运行时配置的部分。
...
/// 云端配置 bundle 服务，协调 backend、缓存与鉴权恢复。
    ...
    C: BundleClient + 'static,
{
    /// 创建一个新的服务实例。
    ///
    /// `codex_home` 用于定位缓存文件目录；`timeout` 限制单次加载流程的总耗时。
    pub(crate) fn new(
        auth_manager: Arc<AuthManager>,
        client: Arc<C>,
        codex_home: PathBuf,
    ...
    /// 在 `timeout` 时间内加载启动期 bundle。
    ///
    /// 超时或失败时返回 [`CloudConfigBundleLoadError`]，并上报 `load` 指标。
    ...
    async fn load_startup_bundle(
        ...
        // 启动期优先使用有效且身份匹配的缓存条目；仅在缓存未命中或缓存内容无效时才向后端请求。
    ...
    /// 后台循环：周期性从后端刷新缓存。
    ///
    /// 每隔 [`CLOUD_CONFIG_BUNDLE_CACHE_REFRESH_INTERVAL`] 触发一次刷新，
    /// 单次刷新超时不会终止循环，仅记录错误并保留现有缓存。
