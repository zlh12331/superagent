//! 通过系统 `git` 二进制应用 unified diff 的辅助工具集。
//!
//! 入口为 [`apply_git_patch`]：将 diff 写入临时文件，调用 `git apply` 并附带合适的
//! 参数，然后解析命令输出为结构化信息。调用方可通过 [`ApplyGitRequest::preflight`]
//! 启用 dry-run 模式，在不实际改动工作树的前提下检视即将变更的路径。
//!
//! 该模块还会尽力将工作树中已存在的路径暂存到 index，以便 `git apply --3way`
//! 在回退场景下能正确执行三方合并。
...
/// 调用 [`apply_git_patch`] 所需的输入参数。
///
/// 字段含义：
/// - `cwd`：执行 git 命令的工作目录（通常为仓库根或其子目录）
/// - `diff`：unified diff 文本
/// - `revert`：是否以反向（`-R`）方式应用补丁
/// - `preflight`：是否仅做 dry-run 检查（`--check`），不改动工作树
#[derive(Debug, Clone)]
...
/// [`apply_git_patch`] 的执行结果，包含从 stdout/stderr 中归纳出的路径信息。
///
/// 字段含义：
/// - `exit_code`：git 进程的退出码
/// - `applied_paths`：成功应用的路径集合
/// - `skipped_paths`：被跳过（如缺少 index 条目）的路径集合
/// - `conflicted_paths`：产生冲突的路径集合
/// - `stdout` / `stderr`：原始命令输出，便于排查问题
/// - `cmd_for_log`：用于日志记录的可读命令字符串
#[derive(Debug, Clone)]
...
/// 通过调用 `git apply` 将 unified diff 应用到目标仓库。
///
/// 当 [`ApplyGitRequest::preflight`] 为 `true` 时，等价于执行 `git apply --check`：
/// 工作树不会被修改，但仍会解析命令输出以便诊断。
///
/// # 参数
/// - `req`：应用补丁所需的请求参数
///
/// # 返回
/// 成功时返回 [`ApplyGitResult`]；若 git 进程无法启动或 IO 失败则返回 [`io::Error`]。
pub fn apply_git_patch(req: &ApplyGitRequest) -> io::Result<ApplyGitResult> {
    let git_root = resolve_git_root(&req.cwd)?;

    // 将 unified diff 写入临时文件
    let (tmpdir, patch_path) = write_temp_patch(&req.diff)?;
    // 让 tmpdir 存活到函数结束，确保临时文件在执行期间始终存在
    let _guard = tmpdir;

    if req.revert && !req.preflight {
        // 反向应用前先暂存工作树中存在的路径，避免 index 与工作树不一致导致回退失败
    ...
    // 可选项：通过环境变量追加 git config 配置（默认关闭）
...
/// 创建临时目录并将 diff 文本写入其中的 `patch.diff` 文件。
///
/// 返回临时目录句柄与补丁文件路径。调用方需保持临时目录存活，直到 git 命令执行完毕。
...
/// 在指定目录下执行 git 命令，返回退出码与 stdout/stderr。
...
/// 对 shell 参数进行最小化转义。
///
/// 当字符串仅由安全字符组成时不加引号；否则使用单引号包裹，并按 POSIX 规则转义内部单引号。
...
/// 构造用于日志记录的可读命令字符串，形如 `(cd <cwd> && git <args>)`。
...
/// 从 diff 文本的 `diff --git` 段落头部提取所有被引用的路径。
///
/// 会去掉 `a/`、`b/` 前缀以及 `/dev/null` 占位符，并按字典序去重返回。
...
fn parse_diff_git_paths(line: &str) -> Option<(String, String)> {
    // 逐字符解析 `diff --git a/path b/path` 头部中的两个路径 token
...
// 读取一个 diff 头部路径 token：支持空白分隔与 C 风格引号转义
...
// 规范化 diff 路径：去掉前缀、忽略 `/dev/null` 占位
...
/// 仅暂存 diff 中存在于磁盘上的文件路径。
///
/// 在反向应用补丁前调用，避免 index 与工作树不一致导致 `git apply -R` 失败。
/// 该函数为尽力而为（best-effort）：即便 `git add` 返回非零也不会报错。
pub fn stage_paths(git_root: &Path, diff: &str) -> io::Result<()> {
    ...
    // 暂存失败不视为致命错误，故即便非零退出也返回 Ok
    Ok(())
}

// ============ 从 VS Code（TypeScript 实现）移植的解析器 ============

/// 将 `git apply` 的输出解析为已应用/已跳过/冲突三类路径分组。
///
/// # 参数
/// - `stdout`：git 命令的标准输出
/// - `stderr`：git 命令的标准错误
///
/// # 返回
/// 元组 `(applied, skipped, conflicted)`，分别对应三类路径列表。
/// 最终优先级：冲突 > 已应用 > 已跳过。
