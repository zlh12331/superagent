...
/// 已安装的 conversation-history replacement boundary。
///
/// Duration-bearing upstream requests 存在于 `CompactionRequest` 中。该对象
/// 是 replacement history 成为 live thread history 的 checkpoint。
/// boundary marker 与 model-visible summary 是独立的 conversation
/// items：marker 标记 history 在何处被替换，而 summary 在 compact endpoint
/// 返回 summary 时是 `replacement_item_ids` 的一部分。
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// runtime 观察到的 Terminal result。
///
/// 这是 debugger/runtime output。它不能证明 model 看到了相同的
/// bytes；通过 `TerminalModelObservation` 关联 model-visible call/output items。
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
