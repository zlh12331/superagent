...
/// 一个 logical transcript item 或 transcript boundary。
///
/// reducer 主要从 inference request 与 response payloads 构建 conversation items。
/// Runtime objects 可列于 `produced_by` 中，但不得改写 item body 所述
/// model 看到的内容。结构性 items（如 compaction markers）存在于同一
/// ordered list 中，使 conversation views 可显示 live history 在何处变更。
...
/// 说明 conversation item 的来源。
///
/// 在 call site 刻意为复数：一个 function output 可同时是
/// model-visible conversation 与 runtime tool call 的产物。
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case", tag = "type")]
