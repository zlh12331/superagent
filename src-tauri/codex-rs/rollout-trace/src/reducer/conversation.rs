...
impl TraceReducer {
    ...
    /// 将 compaction checkpoint payload reduce 为已安装的 replacement history。
    ///
    /// 返回的 ids 让 compaction reducer 记录 boundary marker
    /// 与后续 full requests 应 reconcile 的 snapshot。
    pub(super) fn reduce_compaction_checkpoint(
        ...
        let marker_item_id = self.create_conversation_item(
            ...
            NormalizedConversationItem {
                ...
                agent_message: None,
                // summary 是独立的 model/provider-visible item。保持 marker body
                // 为空，使 transcript renderers 不会将 boundary 误认为 prompt content。
...
/// compaction checkpoint 安装时产出的 conversation ids。
///
/// marker item 记录 boundary，而 replacement items 是后续 full requests
/// 应视为 baseline 的 live history。
