...
struct TraceReducer {
    ...
    /// compaction 安装但尚未在 sampling request 中出现的 replacement snapshot。
    ///
    /// compaction 后的首个 full request 应与已安装的 replacement
    /// history 比对，而非与 pre-compaction request 比对。这使重复的 prefix/context
    /// messages 成为新的 post-compaction conversation items，同时仍复用实际成为
    /// live history 的 summary/replacement items。
