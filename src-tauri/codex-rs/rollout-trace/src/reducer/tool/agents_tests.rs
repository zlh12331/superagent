...
fn agent_result_edge_falls_back_to_child_thread_without_result_message() -> anyhow::Result<()> {
    ...
    // child 收到了其 task 但未产生 assistant output。Failed
    // child tasks 仍可通过 AgentStatus 通知 parent，因此
    // inbound task 不得被误认为 child 的 result。
    start_thread(
        &writer,
        "019d0000-0000-7000-8000-000000000002",
        "/root/child",
    ...
    // parent 确实收到 failure notification 作为 model-visible
    // mailbox item。即使 source 回退到 child thread，
    // target 也应保持该精确的 parent-side ConversationItem。
    start_agent_turn(&writer, "turn-root-1")?;
    let delivered = inter_agent_message(
        "/root/child",
        "/root",
