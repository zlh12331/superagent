...
impl TraceReducer {
    /// 插入一个 thread 并从可选 metadata 派生其 multi-agent identity。
    ///
    /// raw event 携带 denormalized agent path；当 v2 subagent metadata
    /// 存在时，该 metadata 为权威来源，因为它还驱动 spawn edges 与 task names。
