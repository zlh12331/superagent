...
/// 启用本地 trace-bundle 录制的环境变量。
///
/// 值为 root directory。每个独立的 root session 获得一个 child bundle
/// 目录。spawned child threads 共享其 root session 的 bundle，因此一个
/// reduced `state.json` 即可描述整个多 agent rollout tree。
pub const CODEX_ROLLOUT_TRACE_ROOT_ENV: &str = "CODEX_ROLLOUT_TRACE_ROOT";

/// 在 thread/session 启动时一次性捕获的 metadata。
///
/// 该 payload 刻意为 operational 而非 reduced：它是一个 raw payload，
/// 后续 reducer 可在 reduced thread model 演进时从中挖掘信息。
#[derive(Serialize)]
...
impl ThreadTraceContext {
    ...
    /// 在已知目录中启动 root trace。
    ///
    /// 公开供测试使用，使测试无需修改进程环境即可获得 replayable trace bundles。
    ...
    /// 将 v2 child-to-parent 完成消息作为显式 graph edge 发出。
    ///
    /// 该 notification 是从 completed child turn 到 parent mailbox 的 runtime delivery，
    /// 而非 child 执行的 tool call。直接记录可保留时间与来源，无需 reducer
    /// 从后续 parent prompt snapshot 推断该 edge。
    ...
    /// 为一个 Codex turn 构建可复用的 inference trace context。
    ///
    /// 返回的 context 刻意还不是 "an inference call"。transport code 拥有
    /// retry/fallback 尝试，仅在构建好该次尝试的具体 request payload 后
    /// 才调用 `start_attempt`。
    ...
    /// 为一个 checkpoint 构建远程 compaction trace context。
    ///
    /// Rollout tracing 目前仅对 remote compaction 拥有一等公民 checkpoint model。
    /// compact endpoint 是一个 model-facing request，其输出替换 live history，
    /// 因此既需要 request/response attempt events，也需要在 processed replacement
    /// history 安装后发出后续 checkpoint event。
