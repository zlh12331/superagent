//! Skills 系统模块：管理内置系统 skills 的安装与缓存。
//!
//! 该 crate 负责将编译期嵌入的系统 skills 目录写入 `CODEX_HOME/skills/.system`
//! 磁盘缓存。为避免每次启动都重复写入，使用 marker 文件记录嵌入式目录的
//! 指纹；当指纹匹配时跳过安装。
//!
//! 核心函数：[`install_system_skills`]、[`system_cache_root_dir`]。
//! 核心错误类型：[`SystemSkillsError`]。
...
/// 将嵌入式系统 skills 安装到 `CODEX_HOME/skills/.system`。
///
/// 首先清除已有的系统 skills 目录，然后将嵌入式 skills 目录写入目标位置。
///
/// 为避免每次启动都做无用功，安装完成后会写入一个 marker 文件，其中包含
/// 嵌入式目录的指纹。当 marker 匹配时，跳过本次安装。
...
/// 将嵌入式 `include_dir::Dir` 写入磁盘 `dest` 路径下。
///
/// 保留嵌入式目录的原始结构。
...
/// 系统 skills 安装过程中可能发生的错误。
#[derive(Debug, Error)]
pub enum SystemSkillsError {
    /// IO 错误：在执行 `action` 描述的操作时发生。
