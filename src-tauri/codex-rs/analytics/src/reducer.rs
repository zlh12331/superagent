//! Analytics 事件 reducer。
//!
//! [`AnalyticsReducer`] 是 analytics 事件处理的核心：
//! 接收 [`AnalyticsFact`] 输入，维护 turn / thread 级别的事件状态，
//! 在适当时机（如 turn 完成、item 完成）将其转换为 [`TrackEventRequest`]
//! 列表并交由客户端上报。
//!
//! reducer 内部维护：
//! - turn 级别的 token usage、profile、resolved config
//! - item 级别的生命周期、review 状态、耗时
//! - guardian review 的会话上下文
//! - hook 执行的聚合状态
...
/// 返回用于 skill ID 构造的归一化路径。
///
/// - 仓库作用域 skill 使用相对仓库根的路径。
/// - 用户 / 管理员 / 系统 skill 使用绝对路径。
