//! Analytics fact 类型定义。
//!
//! [`AnalyticsFact`] 是 reducer 的输入，表示一次待处理的 analytics 事件，
//! 可能直接来自 app-server 协议事件，也可能是自定义的 [`CustomAnalyticsFact`]。
//!
//! 同时定义 turn、plugin、skill、hook、compaction、goal 等业务 fact 结构。
...
pub(crate) enum AnalyticsFact {
    ...
    // 不直接出现在 app-server 协议表面、或在当前分支上需要较大协议改造的 fact。
