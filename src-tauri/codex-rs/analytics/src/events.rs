//! Analytics 事件请求与参数定义。
//!
//! 定义 [`TrackEventRequest`] 及其所有变体对应的请求结构、参数结构，
//! 用于序列化为上报到后端的 JSON payload。
//!
//! 同时包含 guardian 审查、turn、tool item、plugin 等事件相关的
//! 枚举与上下文类型。
...
pub enum GuardianApprovalRequestSource {
    /// 由主 Codex turn 直接发起的审批请求。
    MainTurn,
    /// 由子代理委托发起的审批请求，经父会话路由至 guardian 审查。
    DelegatedSubagent,
}
...
impl GuardianReviewTrackContext {
    ...
    pub(crate) fn event_params(
        ...
        GuardianReviewEventParams {
            ...
            // TODO(rhan-oai): 待补充嵌套 Guardian review session 的工具调用计数。
...
pub(crate) struct CodexToolItemEventBase {
    pub(crate) thread_id: String,
    pub(crate) turn_id: String,
    /// App-server ThreadItem.id。对于工具发起的 item，通常对应
    /// 上游 core 的 call_id。
    ...
    // 观测到的 item 生命周期耗时。对于 app-server 只能看到上游流程一部分的工具，
    // 该值可能低于实际端到端执行耗时。
...
pub(crate) struct CodexTurnEventParams {
    ...
    pub(crate) session_id: String,
    pub(crate) turn_id: String,
    // TODO(rhan-oai): 待 turn/start 调用点支持队列/默认提交类型后补充该字段，
    // 当前始终上报为 None。
...
fn analytics_hook_status(status: HookRunStatus) -> HookRunStatus {
    match status {
        // Running 状态在此处不应出现，防御性地归一化为 Failed。
