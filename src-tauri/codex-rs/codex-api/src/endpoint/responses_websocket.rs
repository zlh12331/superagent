...
impl ResponsesWebsocketClient {
    ...
    /// 打开一个 WebSocket 连接，仅维持到验证 upgrade 响应完成即可。
    ///
    /// 该 probe 使用与真实 Responses WebSocket 连接相同的 URL 构造、headers、authentication、TLS
    /// 以及 custom-CA 路径，但不会发送任何 request frame。当 HTTP 101 upgrade 成功后，
    /// 会短暂等待服务器是否立即发送 close frame，以便诊断逻辑能够区分：
    /// 可用连接与因策略被拒绝而立即关闭的连接。
