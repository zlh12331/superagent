//! Terminal detection 工具集。
//!
//! 本模块将 terminal metadata 输入 OpenTelemetry user-agent logging，
//! 并用于 TUI 中 terminal 专属的配置选择。
...
pub enum Multiplexer {
    ...
    Tmux {
        /// tmux version string，当 `TERM_PROGRAM=tmux` 时可用。
        ///
        /// 该值派生自 `TERM_PROGRAM_VERSION` 环境变量。
...
/// `xterm-256color`).
///
/// 该信息仅在 tmux 下运行时可用，用于将 session 归因到底层 terminal
/// 而非 tmux 本身。
...
///
/// 该 trait 的存在是为了在测试中伪造环境变量访问，便于注入 mock 环境。
trait Environment {
