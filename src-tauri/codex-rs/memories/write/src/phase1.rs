//! Phase 1（stage-1 extraction）实现。
//!
//! 该模块按严格步骤顺序执行 memory Phase 1：
//! 1. claim 符合条件的 rollout 任务
//! 2. 构造 stage-1 请求上下文
//! 3. 并发执行 stage-1 extraction 任务
//! 4. 上报指标与日志
...
/// 单个 stage-1 任务执行结果，包含 outcome 与 token usage。
struct JobResult {
    outcome: JobOutcome,
    token_usage: Option<TokenUsage>,
}

/// stage-1 任务的三种结局：成功有输出、成功无输出、失败。
...
struct Stats {
    ...
    /// 失败的任务数。
    failed: usize,
    /// 累计 token usage（任一任务上报则视为有值）。
    total_token_usage: Option<TokenUsage>,
}

/// Phase 1 模型输出 payload。
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields)]
struct StageOneOutput {
    /// 单条 rollout 的详细 markdown 原始 memory。
...
/// 按 strict 步骤顺序执行 memory Phase 1：
/// 1) claim 符合条件的 rollout 任务
/// 2) 构造 stage-1 请求上下文
/// 3) 并发执行 stage-1 extraction 任务
/// 4) 上报指标与日志
pub async fn run(context: Arc<MemoryStartupContext>, config: Arc<Config>) {
    ...
    if claimed_candidates.is_empty() {
        // 没有 candidate 任务时上报 skipped 指标后直接返回。
...
/// 修剪过期且未使用的 "dead" raw memories，控制 DB 体积。
///
/// 该操作不消耗 token，可在 quota 检查之前执行。
...
/// 返回用于约束 Phase 1 模型输出的 JSON schema。
...
/// 从 state DB claim 一批可执行的 stage-1 启动任务。
///
/// 返回 `None` 表示 state db 不可用或 claim 失败；空 `Vec` 表示当前没有可执行任务。
...
/// 构造 stage-1 请求上下文（模型名、reasoning effort 等）。
async fn build_request_context(
    context: &MemoryStartupContext,
    config: &Config,
) -> StageOneRequestContext {
    // 优先使用配置中显式指定的模型，否则使用 provider 推荐的 memory extraction 模型。
...
/// 并发执行所有 claimed 任务，返回每个任务的执行结果。
///
/// 使用 `buffer_unordered` 控制并发上限为 `stage_one::CONCURRENCY_LIMIT`。
async fn run_jobs(
...
mod job {
    ...
    /// 执行单个 stage-1 任务，返回 [`JobResult`]。
    ///
    /// 流程：sampling -> 结果分类（成功有输出 / 无输出 / 失败）-> 持久化。
    ...
    /// 加载 rollout 并执行实际的模型采样。
    ///
    /// 返回 `(StageOneOutput, token_usage)`；模型输出会经过密钥脱敏处理。
    async fn sample(
        context: &MemoryStartupContext,
        config: &Config,
        rollout_path: &Path,
    ...
    mod result {
        ...
        /// 标记任务成功但无输出。
        ///
        /// 返回 [`JobOutcome::SucceededNoOutput`]；DB 不可用或更新失败时返回 [`JobOutcome::Failed`]。
        ...
        /// 标记任务成功并写入 raw memory / summary / slug。
        ///
        /// 返回 [`JobOutcome::SucceededWithOutput`]；DB 不可用或更新失败时返回 [`JobOutcome::Failed`]。
    ...
    /// 将过滤后的 stage-1 memory items 序列化为字符串，供 prompt 使用。
    ///
    /// 过程中会调用 [`sanitize_response_item_for_memories`] 剔除无关项，
    /// 并对最终字符串做密钥脱敏。
    pub(super) fn serialize_filtered_rollout_response_items(
        ...
        let filtered = items
            ...
            .filter_map(|item| match item {
                ...
                // 元数据与 compacted 等 item 不参与 memory 提取。
    ...
    /// 清理单条 [`ResponseItem`]，决定其是否应进入 memory 提取输入。
    ///
    /// - developer 角色消息直接丢弃。
    /// - user 角色消息会剔除 AGENTS.md instructions、skill 等上下文片段。
    /// - 其他 item 通过 [`should_persist_response_item_for_memories`] 判断。
    fn sanitize_response_item_for_memories(item: &ResponseItem) -> Option<ResponseItem> {
        ...
        // user 消息：剔除被标记为上下文的片段（如 AGENTS.md、skill）。
        let content = content
            .iter()
    ...
    /// 判断 user content 片段是否为 memory 排除的上下文片段（AGENTS.md instructions / skill）。
    ...
    /// 判断 `text` 是否以 `start_marker` 开头并以 `end_marker` 结尾（不区分大小写）。
...
/// 聚合一组任务结果，生成 [`Stats`]。
///
/// 同时累加所有任务的 token usage；任一任务上报 token usage 即视为有值。
...
/// 上报 Phase 1 相关指标。
///
/// 包括任务计数（按 status 分桶）、output 计数与 token usage histogram。
