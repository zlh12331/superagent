//! 将 tracing 日志导出到本地 SQLite 日志数据库。
//!
//! 本模块提供一个 `tracing_subscriber::Layer`，捕获 tracing 事件，
//! 将每个事件格式化为 [`LogEntry`]，并发送到一个有界后台队列。
//! 后台任务批量插入到专用的 `logs` SQLite 数据库，以降低日志开销。
//!
//! ## 用法
//!
//! ```no_run
...
/// 返回默认的日志过滤目标配置。
///
/// 默认开启 TRACE 级别，并显式关闭 `log`、`codex_otel.log_only`
/// 与 `codex_otel.trace_safe` 三个 target 以避免重复记录。
...
/// 可刷写队列已接收条目的 tracing 日志 writer trait。
///
/// 实现应保证 `Layer::on_event` 在普通日志事件下不阻塞；
/// `flush` 应等待此前已入队的条目被 writer 处理完毕。
...
/// 将 tracing 事件写入本地 SQLite 日志库的 layer。
pub struct LogDbLayer {
    sender: mpsc::Sender<LogDbCommand>,
    process_uuid: String,
}

/// 启动日志 layer 并返回 [`LogDbLayer`]。
///
/// # 参数
/// - `state_db`: state runtime 句柄
    ...
    S: tracing::Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: tracing_subscriber::layer::Context<'_, S>) {
        ...
        // `tracing-log` 会在分发给 tracing target 为 `log` 的事件之前
        // 用原始 log target 检查过滤器，因此外层 target 过滤器无法可靠地
        // 拒绝这些桥接事件，这里需要显式跳过。
        if metadata.target() == "log" {
            return;
        }

        // SDK 每秒每进程会发出 DEBUG 级别的 timer meta-event；
        // 在实测的高扇出 Codex 环境中，这类事件占保留日志的 30% 以上。
