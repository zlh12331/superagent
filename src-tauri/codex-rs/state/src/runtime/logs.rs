...
"#,
    ...
    /// 查询一组 thread 的反馈日志，按 SQLite 保留预算裁剪。
    ///
    /// # 参数
    /// - `thread_ids`: thread ID 字符串切片
    ///
    /// # 返回值
    /// 返回拼接后的反馈日志字节流；若查询失败则返回 `Err`。
    pub async fn query_feedback_logs_for_threads(
        ...
        // 先在 SQL 层限制取回行数，避免对超额保留的 partition 把所有行加载到内存；
        // 格式化后再按整行字节精确裁剪。
        let mut builder = QueryBuilder::<Sqlite>::new(
            r#"
...
"#,
    ...
    /// 查询单个 thread 的反馈日志，按 per-thread SQLite 保留预算裁剪。
    ///
    /// # 参数
