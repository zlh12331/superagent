...
/// thread goals 的 SQLite 存储，每个 thread 至多对应一个 goal。
#[derive(Clone)]
...
/// 对 thread goal 的部分更新字段。
pub struct GoalUpdate {
    /// 新目标文本（若提供）。
    pub objective: Option<String>,
    /// 新状态（若提供）。
    pub status: Option<crate::ThreadGoalStatus>,
    /// 新 token 预算（`Some(None)` 表示清除预算）。
    pub token_budget: Option<Option<i64>>,
    /// 期望的现有 goal ID，用于乐观并发控制。
    pub expected_goal_id: Option<String>,
}

/// goal 记账结果：要么保持不变，要么已更新。
pub enum GoalAccountingOutcome {
    /// 未发生变更，包含变更前的 goal（可能为 `None`）。
    Unchanged(Option<crate::ThreadGoal>),
    /// 已更新，返回更新后的 goal。
    Updated(crate::ThreadGoal),
}

/// goal 记账的模式，控制哪些状态的 goal 需要参与记账。
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum GoalAccountingMode {
    /// 仅对状态为 Active 的 goal 记账。
    ActiveStatusOnly,
    /// 仅对存在且 Active 的 goal 记账。
    ActiveOnly,
    /// 对 Active 或 Complete 的 goal 记账。
    ActiveOrComplete,
    /// 对 Active 或 Stopped 的 goal 记账。
    ActiveOrStopped,
}

impl GoalStore {
    /// 读取指定 thread 的当前 goal。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    ///
    /// # 返回值
    /// 返回 [`ThreadGoal`]；若不存在则返回 `Ok(None)`。
...
WHERE thread_id = ?
    ...
    ///
    /// # 返回值
    /// 返回写入后的 [`ThreadGoal`]；若写入失败则返回 `Err`。
...
RETURNING
    ...
    /// 仅当现有 goal 为 complete 时才插入新 goal。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `objective`: 目标文本
    ...
    ///
    /// # 返回值
    /// 若成功插入则返回 `Ok(Some(ThreadGoal))`；若现有 goal 非 complete 则返回 `Ok(None)`。
...
RETURNING
    ...
    /// 按 [`GoalUpdate`] 部分更新指定 thread 的 goal。
    ///
    /// 若 `expected_goal_id` 与现有 goal 不匹配，则不执行更新并返回 `Ok(None)`。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `update`: 部分更新字段
    ///
    /// # 返回值
    /// 返回更新后的 [`ThreadGoal`]；若未匹配或不存在则返回 `Ok(None)`。
...
WHERE thread_id = ?
  AND (? IS NULL OR goal_id = ?)
    ...
    /// 将指定 thread 的当前 active goal 标记为 paused。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    ///
    /// # 返回值
    /// 返回更新后的 [`ThreadGoal`]；若无 active goal 则返回 `Ok(None)`。
    ...
    /// 将指定 thread 的当前 active goal 标记为 usage_limited。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    ///
...
WHERE thread_id = ?
  AND (
    ...
    ///
    /// # 返回值
    /// 返回被删除的 [`ThreadGoal`]；若不存在则返回 `Ok(None)`。
...
RETURNING
    ...
    /// 对指定 thread 的 goal 进行用量记账（累加 token 与时间消耗）。
    ///
    /// 根据 [`GoalAccountingMode`] 决定哪些状态的 goal 参与记账。
    /// 若累计 token 用量超过预算，会自动将状态推进到 budget_limited/usage_limited。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `time_delta_seconds`: 时间增量（秒）
    /// - `token_delta`: token 增量
    /// - `mode`: 记账模式
    /// - `expected_goal_id`: 可选的乐观并发 goal ID
    ///
    /// # 返回值
    /// 返回 [`GoalAccountingOutcome`]，区分未变更与已更新。
