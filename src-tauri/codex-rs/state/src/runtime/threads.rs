...
WHERE id = ? AND preview = ''
    ...
    /// 持久化或替换已 spawn thread 的方向性 parent-child 边。
...
ON CONFLICT(child_thread_id) DO UPDATE SET
    ...
    /// 列出 `parent_thread_id` 的直接子 thread 中边状态匹配 `status` 的项。
    ...
    /// 列出 `parent_thread_id` 的所有直接子 thread。
    ...
    /// 列出 `root_thread_id` 的后代 thread 中边状态匹配 `status` 的项。
    ///
    /// 后代按深度优先广度优先返回，再按 thread id 排序以保证顺序稳定。
    ...
    /// 列出 `root_thread_id` 的所有后代 thread。
    ///
    /// 后代按深度优先广度优先返回，再按 thread id 排序以保证顺序稳定。
    ...
    /// 按规范 agent path 查找 `parent_thread_id` 的直接子 thread。
...
LIMIT 2
    ...
    /// 按规范 agent path 查找 `root_thread_id` 的后代 thread。
...
ON CONFLICT(child_thread_id) DO NOTHING
    ...
    /// 通过 thread ID 在底层数据库中查找 rollout 路径。
    ...
    /// 查找用户可见标题与 `title` 完全匹配的最新 thread。
    ...
    /// 通过持久化的 spawn 边列出 `parent_thread_id` 的直接子 thread。
    ...
    /// 通过底层数据库列出 thread ID（不扫描 rollout 目录）。
    ...
    ///
    /// # 返回值
    /// 若实际插入则返回 `Ok(true)`；若已存在则返回 `Ok(false)`。
...
ON CONFLICT(id) DO NOTHING
    ...
    /// 设置 thread 的 memory mode 字段。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `memory_mode`: memory mode 字符串
    ///
    /// # 返回值
    /// 若实际更新则返回 `Ok(true)`；否则返回 `Ok(false)`。
    ...
    /// 更新 thread 的 recency_at 时间戳，单调递增。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `recency_at`: 期望的 recency 时间
    ///
    /// # 返回值
    /// 若实际更新则返回 `Ok(true)`；否则返回 `Ok(false)`。
...
WHERE id = ?
    ...
    /// 为 thread 列表游标排序分配持久化的 `updated_at` 值。
    ///
    /// 我们保留一个进程本地高水位线，使热点 rollout 写入无需每次查询 SQLite
    /// 即可获得唯一、单调递增的毫秒时间戳。较早的 backfill/repair 时间戳
    /// 原样放行，使历史排序仍与 rollout 文件 mtime 保持一致。
...
fn allocate_thread_timestamp(
    ...
    let allocated = loop {
        ...
        // 较早的时间戳来自保留 rollout mtime 的 backfill/repair 路径。
        // 不要仅因为本进程见过更新的写入就把历史行往前推。
        if candidate.saturating_add(1000) <= current {
            break candidate;
        }

        // 与当前高水位线处于同一热点秒级桶内。分配下一毫秒，使时间戳在
        // 进程内保持唯一且可按游标排序。
...
impl StateRuntime {
    /// 更新 thread 的 git 信息（commit sha、分支、origin URL）。
    ///
    /// 每个字段使用 `Option<Option<&str>>` 表达三态：
    /// - `None`：不更新该字段
    /// - `Some(None)`：将该字段置为 NULL
    /// - `Some(Some(value))`：将该字段更新为 value
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `git_sha`: commit sha 三态
    /// - `git_branch`: 分支三态
    /// - `git_origin_url`: origin URL 三态
    ///
    /// # 返回值
    /// 若实际更新则返回 `Ok(true)`；否则返回 `Ok(false)`。
...
ON CONFLICT(id) DO UPDATE SET
    ...
    /// 通过底层数据库增量应用 rollout items。
    ///
    /// # 参数
    /// - `builder`: thread 元数据构造器
    /// - `items`: rollout item 切片
    /// - `new_thread_memory_mode`: 可选的新 memory mode
    /// - `updated_at_override`: 可选的 updated_at 覆盖值
    ///
    /// # 返回值
    /// 成功返回 `Ok(())`；若写入失败则返回 `Err`。
    ...
    /// 通过底层数据库将 thread 标记为已归档。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `rollout_path`: 归档后的 rollout 路径
    /// - `archived_at`: 归档时间
    ///
    /// # 返回值
    /// 成功返回 `Ok(())`；若写入失败则返回 `Err`。
    ...
    /// 将一个 thread 标记为未归档（unarchived），并更新底层记录。
    ///
    /// # 参数
    /// - `thread_id`: thread ID
    /// - `rollout_path`: 未归档后的 rollout 路径
    ///
    /// # 返回值
    /// 成功返回 `Ok(())`；若写入失败则返回 `Err`。
    ...
    /// 按 ID 删除单个 thread 及其所有关联状态。
    ///
    /// 内部委托给 [`delete_threads_strict`]，返回受影响的行数。
    pub async fn delete_thread(&self, thread_id: ThreadId) -> anyhow::Result<u64> {
        self.delete_threads_strict(&[thread_id]).await
    }

    /// 批量删除一组 thread 及其所有关联状态。
    ///
    /// spawn edges 与 thread 行最后才删除，这样即便删除过程中途失败，
    /// 仍可凭借剩余的状态重新发现同样的 spawned 子树并重试删除。
    ///
    /// # 参数
    /// - `thread_ids`: 待删除的 thread ID 列表
    ///
    /// # 返回值
    /// 成功删除的 thread 行数。
    pub async fn delete_threads_strict(&self, thread_ids: &[ThreadId]) -> anyhow::Result<u64> {
        ...
        for thread_id_string in &thread_id_strings {
            for parent_thread_id_string in &thread_id_strings {
                // 若 job runner 与 worker 同时被删除，重新入队 worker item
                // 会导致一个 running job 没有循环消费它。
                sqlx::query(
                    r#"
UPDATE agent_jobs
...
"#,
    ...
    // 否则 SQLite 可能重排这些 join，并在检查关系之前先扫描全局时间戳索引。
    // 使用 CROSS JOIN 强制让选择性更高的 edge/subtree 遍历排在前面。
    ...
    let order_by_index = match (relation_filter, filters.cwd_filters) {
        // 关系列表的规模预期远小于全局 thread 表。
        // 优先使用 spawn-edge 索引并对匹配的子树排序，而不是扫描时间戳索引
        // 直到凑够相关的 thread。
        (Some(_), _) => OrderByIndex::Disabled,
        // 多 cwd 列表虽然支持，但截至撰写时尚无生产环境使用。
        // 保留其查询计划，避免全局时间戳索引将 cwd 过滤退化为全表扫描。
...
/// 列出 thread 时使用的过滤选项集合。
///
/// 该结构以引用形式聚合多个过滤维度，避免在调用栈中传递一长形参；
/// 由 [`push_thread_filters`] 消费并拼装到 SQL `WHERE` 子句中。
#[derive(Clone, Copy)]
pub struct ThreadFilterOptions<'a> {
    /// 是否仅返回已归档的 thread（`true` 为仅归档，`false` 为仅未归档）。
    pub archived_only: bool,
    /// 允许的 thread source 列表；为空表示不按 source 过滤。
    pub allowed_sources: &'a [String],
    /// 允许的 model provider 列表；`None` 表示不过滤。
    pub model_providers: Option<&'a [String]>,
    /// 允许的 cwd 列表；`None` 表示不过滤，`Some([])` 表示匹配空集（即返回空结果）。
    pub cwd_filters: Option<&'a [PathBuf]>,
    /// 分页锚点（基于时间戳 + 可选 thread ID 的游标）。
    pub anchor: Option<&'a crate::Anchor>,
    /// 排序字段（CreatedAt / UpdatedAt / RecencyAt）。
    pub sort_key: SortKey,
    /// 排序方向（升序 / 降序）。
    pub sort_direction: SortDirection,
    /// 标题与预览的搜索关键词；`None` 表示不进行文本搜索。
...
/// 控制 SQLite 是否允许利用排序列的索引来满足 `ORDER BY`。
///
/// 禁用时会为排序列添加一元 `+`，保留排序语义但阻止仅基于时间戳的索引
/// 击败选择性更高的过滤索引。
#[derive(Clone, Copy)]
