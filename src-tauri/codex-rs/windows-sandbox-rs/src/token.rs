...
/// 创建一个 restricted token，包含所有传入的 capability SIDs。
///
/// 该函数基于 base_token 创建受限令牌，并将指定的 capability SIDs 加入其中，
/// 用于授予 sandbox 所需的特定 capability 权限。
///
/// # Safety
/// 调用方必须关闭返回的 token handle，否则会造成资源泄漏；base_token 必须是一个有效的
/// primary token，否则创建过程将失败。
...
/// 创建一个 restricted token，包含所有传入的 capability SIDs 以及 token user SID。
///
/// 该函数用于 elevated sandbox backend，此时 token user 是 dedicated 的
/// sandbox 账户，而非真实登录的用户。将 user SID 一并加入 token 可确保
/// sandbox 进程以专用账户身份访问需要用户身份的资源。
///
/// # Safety
/// 调用方必须关闭返回的 token handle，否则会造成资源泄漏；base_token 必须是一个有效的
/// primary token，否则创建过程将失败。
