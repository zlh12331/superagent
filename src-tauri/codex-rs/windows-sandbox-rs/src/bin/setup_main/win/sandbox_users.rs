...
// 在 provisioning 开始之前，创建带有受保护 ACL 的最终 marker 路径。空文件
// 会在 setup 进行期间使 readiness checks 失败，sandbox users 无法读取、
// 修改或替换它。一旦所有 setup 步骤成功完成，`commit_setup_marker` 会写入有效的
// marker 内容，但不改变文件的 ACL。
