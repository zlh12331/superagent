...
/// raw Win32 handles 的小型 RAII wrapper。
///
/// elevated runner 有一些 early-return 路径，我们在这些路径中获取 token、job 或 pipe
/// handle，然后在准备 child 时可能失败。将这些 handles 保存在 guard 中可以使
/// 错误路径更直观，并堵住之前泄漏它们的漏洞。
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    ...
    // token helpers 仍然接受 raw SID pointers，但我们尽可能长时间地将所有权
    // 保留在 `LocalSid` wrappers 中。这样，在 SID 解析之后、child 完全 spawn 之前的
    // 任何失败都会自动释放 backing LocalAlloc 内存。
