...
pub struct ModelProviderInfo {
    ...
    /// 该 provider 期望使用的 wire protocol。
    ///
    /// 决定请求与响应的序列化格式（如 Responses API 或 Chat Completions），
    /// 影响 HTTP 请求体构造与响应解析方式。
    #[serde(default)]
    ...
    /// 该 provider 是否需要 OpenAI API Key 或 ChatGPT login token？
    ///
    /// 若为 true，首次运行时向用户展示登录界面，登录偏好与 token/key
    /// 持久化存储于 auth.json。若为 false（默认值），跳过登录界面，
    /// API key（如需）从 `env_key` 环境变量读取。
    #[serde(default)]
