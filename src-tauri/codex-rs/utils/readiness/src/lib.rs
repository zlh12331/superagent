...
pub struct ReadinessFlag {
    ...
    next_id: AtomicI32,
    /// 当前 active subscriptions 的集合。每个 subscription 通过唯一的 token
    /// 标识，当集合非空时表示仍有订阅者在等待 readiness 信号。
...
impl ReadinessFlag {
    /// 创建一个新的、尚未就绪（not-yet-ready）的 flag。
    /// 初始状态下 `ready` 为 false，调用方需在完成初始化后显式调用
    /// `set_ready` 来通知所有等待者。
    pub fn new() -> Self {
