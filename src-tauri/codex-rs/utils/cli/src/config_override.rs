//! 为 Codex CLI 工具集提供对 `-c key=value` overrides 的共享支持。
//!
//! 本模块提供了一个 [`CliConfigOverrides`] struct，可通过 `#[clap(flatten)]`
//! 嵌入到 `clap` 派生的 CLI struct 中。每次出现的 `-c key=value`（或
//! `--config key=value`）都会以原始字符串的形式被收集。模块还提供了辅助
//! 方法，用于将原始字符串转换为 key/value 对，以及将其应用到代表配置树的
//! 可变 `serde_json::Value` 上。
