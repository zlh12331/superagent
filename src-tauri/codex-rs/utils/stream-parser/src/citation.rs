...
/// 针对 `<oai-mem-citation>...</oai-mem-citation>` 标签的 stream parser。
///
/// 这是 [`InlineHiddenTagParser`] 的一个 thin convenience wrapper。它将 citation
/// 的正文内容以纯字符串形式返回，同时从可见文本中去除 citation 标签本身。
///
/// 匹配方式为字面量匹配，且不支持嵌套。如果在遇到闭合标签
/// `</oai-mem-citation>` 之前就到达 EOF，parser 会自动闭合该标签，并将已
/// 缓冲的正文作为一条已提取的 citation 返回，确保不会丢失数据。
#[derive(Debug)]
