//! Codex 登录与认证 crate。
//!
//! 提供与 OpenAI/Codex 后端进行交互式登录、令牌存储与刷新、外部认证接入等能力。
//!
//! 主要导出项：
//! - [`CodexAuth`]：统一认证抽象，由 [`auth`] 模块提供。
//! - [`AuthManager`]：集中管理认证状态的共享管理器。
//! - [`LoginServer`] / [`ServerOptions`]：用于交互式 OAuth 回调的本地服务器。
//! - [`DeviceCode`] / [`run_device_code_login`]：设备码登录流程。
//! - [`TokenData`]：JWT 令牌数据的解析与存储模型。
//! - [`AuthEnvTelemetry`]：登录相关环境变量的遥测采集。
