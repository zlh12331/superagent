...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    // 在后台运行登录服务器（CLI 登录流程会通过本地回调与该服务器通信）
    ...
    // 以下断言保留是因为旧的 OAuth 流程会用 token 换取 API key
    // （详见 server.rs 中的 obtain_api_key 实现）。
    // 待该旧机制从代码中移除后，本测试需更新为预期 API key 不再出现在 auth.json 中。
    ...
    // 停止 mock issuer（drop handle 会终止后台 issuer 任务，释放端口）
    drop(issuer_handle);
    Ok(())
}
...
async fn creates_missing_codex_home_dir() -> Result<()> {
    ...
    let tmp = tempdir()?;
    let codex_home = tmp.path().join("missing-subdir"); // 该子目录不存在，用于测试服务器自动创建 codex_home 的行为
