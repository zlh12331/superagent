//! MCP server 配置类型。
//!
//! 定义 codex-rs 可连接的 MCP（Model Context Protocol）server 配置 schema，
//! 包括传输层（stdio / streamable_http）、认证（OAuth / ChatGPT）、环境变量、
//! 超时、工具审批模式等。`RawMcpServerConfig` 用于反序列化与校验，`McpServerConfig`
//! 为运行时使用的规范化形态。
...
pub enum McpServerDisabledReason {
    /// server 已被禁用，但没有更具体的面向用户的原因。
    Unknown,
    /// server 因来自给定 source 的 config requirements 而被禁用。
...
pub enum McpServerTransportConfig {
    ...
    StreamableHttp {
        ...
        /// 实际的 secret 值必须通过环境变量提供，切勿硬编码在配置文件中。
        #[serde(default, skip_serializing_if = "Option::is_none")]
