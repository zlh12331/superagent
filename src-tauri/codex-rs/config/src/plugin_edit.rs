//! Plugin 配置编辑与持久化。
//!
//! 提供对 `config.toml` 中 `[plugins.<key>]` 表的启用/禁用/清除操作。
//! 写入使用 `toml_edit` 保留格式，并支持符号链接跟随与原子写入。
...
/// 批量应用 plugin 配置编辑到 `config.toml`（在阻塞线程上执行）。
///
/// 无编辑时直接返回；有编辑时读取文档、应用全部修改、原子写回。
