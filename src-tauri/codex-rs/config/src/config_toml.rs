//! Codex 核心 schema 配置 TOML 类型。
//!
//! 定义 `config.toml` 的根结构 [`ConfigToml`] 及其嵌套类型（sandbox、
//! MCP、模型 provider、realtime、agents、hooks、features 等）。这些类型
//! 通过 serde + schemars 同时服务于 TOML 反序列化与 JSON Schema 生成，
//! 是用户/云端配置与运行时之间的契约边界。
