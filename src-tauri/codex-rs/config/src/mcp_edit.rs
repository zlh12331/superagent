//! MCP server 配置编辑与持久化。
//!
//! 提供从 `config.toml` 读取全局 MCP server 配置（`load_global_mcp_servers`）
//! 与通过 `ConfigEditsBuilder` 原子写回修改的能力。写入使用 `toml_edit`
//! 以保留现有文档格式（注释、空格等）。
...
/// 从 `codex_home/config.toml` 读取全局 MCP server 配置。
///
/// 文件不存在时返回空 map；`mcp_servers` 表缺失时返回空 map。检测到
/// 内联 `bearer_token`（已废弃）时返回 `InvalidData` 错误，引导用户
/// 改用 `bearer_token_env_var`。
...
/// 检查 MCP server 配置中不存在内联 `bearer_token`。
///
/// 内联 bearer token 已废弃（安全风险），用户应改用 `bearer_token_env_var`
/// 从环境变量读取。发现违规时返回 `InvalidData` 错误。
...
/// 配置编辑构造器：累积修改并在 `apply` 时原子写回 `config.toml`。
///
/// 使用建造者模式：`new` 创建实例，`replace_mcp_servers` 等方法链式叠加
/// 修改，`apply` 在阻塞线程上执行文件 I/O 并写回。
