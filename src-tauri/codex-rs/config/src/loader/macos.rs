//! macOS 平台 managed preferences（MDM）配置加载。
//!
//! 本模块通过 `CFPreferencesCopyAppValue` 读取 macOS managed preferences，
//! 支持 config 与 requirements 两种 payload。payload 以 base64 编码的
//! TOML 字符串形式存储在 `com.openai.codex` application ID 下。
//!
//! 测试时可通过 `override_base64` 参数直接传入 base64 字符串，
//! 绕过 managed preferences 读取。
...
/// managed preferences 的 application ID。
const MANAGED_PREFERENCES_APPLICATION_ID: &str = "com.openai.codex";
/// managed preferences 中 config payload 的 key。
const MANAGED_PREFERENCES_CONFIG_KEY: &str = "config_toml_base64";
/// managed preferences 中 requirements payload 的 key。
const MANAGED_PREFERENCES_REQUIREMENTS_KEY: &str = "requirements_toml_base64";

/// 从 macOS managed preferences 读取的 admin config 层。
#[derive(Debug, Clone)]
...
/// 构造 managed preferences requirements 来源的 `RequirementSource`。
...
/// 加载 macOS managed admin config 层。
///
/// `override_base64` 为 `Some(_)` 时直接解码该 base64 字符串，
/// 绕过 managed preferences 读取（用于测试）。否则通过
/// `spawn_blocking` 在阻塞线程池中读取 managed preferences，
/// 避免阻塞异步运行时。
///
/// # Errors
/// - base64 解码失败
/// - TOML 解析失败
/// - 严格模式下未知字段校验失败
/// - managed preferences 读取任务 join 失败
...
/// 在阻塞线程中从 managed preferences 读取并解析 admin config。
...
/// 加载 macOS managed admin requirements 层。
///
/// 与 `load_managed_admin_config_layer` 类似，但读取 requirements payload
/// 并封装为 `RequirementsLayerEntry`。
///
/// # Errors
/// - base64 解码失败
/// - managed preferences 读取任务 join 失败
...
/// 在阻塞线程中从 managed preferences 读取 requirements payload。
...
/// 通过 `CFPreferencesCopyAppValue` 读取 managed preference 值。
///
/// `key_name` 指定要读取的 key。返回 `Ok(None)` 表示该 key 未设置。
///
/// # Errors
/// 仅可能返回内部 FFI 调用产生的错误（当前实现始终返回 `Ok`）。
...
/// 解析 base64 编码的 managed config TOML。
///
/// 解码 base64 后解析为 `TomlValue`，严格模式下校验未知字段。
/// 根必须为 table，否则返回错误。
///
/// # Errors
/// - base64 解码失败
...
/// 严格模式下校验 managed config TOML 是否包含未知字段。
///
/// 使用 `AbsolutePathBufGuard` 设置基础目录，使相对路径校验与
/// 正常加载使用相同的路径语义。
...
/// 解析 base64 编码的 managed requirements TOML。
///
/// 与 `parse_managed_config_base64` 的区别在于不解析 TOML，
/// 直接返回解码后的字符串，由上层 `RequirementsLayerEntry::from_toml` 解析。
///
/// # Errors
/// - base64 解码失败
/// - UTF-8 解码失败
