//! 配置层加载核心入口。
//!
//! 本模块负责从多个来源（system/cloud/user/profile/project/runtime/managed）
//! 加载并合并 config 与 requirements 层，构造最终的 `ConfigLayerStack`。
//! 子模块：
//! - `layer_io`：从磁盘与 MDM 读取 managed config 层的 I/O 操作。
//! - `macos`：macOS 平台 managed preferences（MDM）配置加载（仅 macOS）。
...
/// Unix 平台下系统级 config.toml 的默认路径。
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/codex/config.toml";

/// Windows 平台下 `ProgramData` 目录的默认回退路径。
#[cfg(windows)]
const DEFAULT_PROGRAM_DATA_DIR_WINDOWS: &str = r"C:\ProgramData";

// 项目级配置来自仓库内容，因此不应允许它决定用户凭据发送到何处、
// 或运行哪些本地命令。这些设置仍可从 user/system/managed/runtime 层配置。
...
/// 从层列表中找出第一个含解析错误的 config 层，返回其 `ConfigError`。
///
/// 仅用于在 project trust 或 project root markers 解析失败时，把更精确的
/// 配置错误（而非原始 toml 错误）返回给调用方。
...
/// 加载并组合所有 config 与 requirements 层，构造最终的 `ConfigLayerStack`。
///
/// 为构造 admin 强制约束的 requirements 层集合，按与 config 层一致的
/// 升序优先级收集，然后用 config 风格的 TOML 合并加上针对 hooks、rules、
/// deny-read 权限、remote sandbox config 的字段特定处理进行组合：
///
/// - system：`/etc/codex/requirements.toml`（Unix）或
///   `%ProgramData%\OpenAI\Codex\requirements.toml`（Windows）
/// - cloud：企业管理的 cloud config bundle requirements
/// - legacy：`managed_config.toml` 重新解释为 `requirements.toml`
/// - admin：managed preferences（*）
///
/// 出于向后兼容，仍会从 `managed_config.toml` 加载并映射为
/// `requirements.toml`。
///
/// config 层按以下顺序构建：
///
/// - admin：managed preferences（*）
/// - system：`/etc/codex/config.toml`（Unix）或
///   `%ProgramData%\OpenAI\Codex\config.toml`（Windows）
/// - cloud：企业管理的 cloud config bundle 片段
/// - user：`${CODEX_HOME}/config.toml`
/// - profile：`${CODEX_HOME}/<name>.config.toml`（被选中时）
/// - cwd：`${PWD}/config.toml`（目录不受信任时加载但禁用）
/// - tree：向上遍历父目录查找 `./.codex/config.toml`（不受信任时加载但禁用）
/// - repo：`$(git rev-parse --show-toplevel)/.codex/config.toml`（不受信任时加载但禁用）
/// - runtime：例如 `--config` 标志、UI 中的 model 选择器
///
/// (*) 仅在 macOS 上通过 managed device profiles 提供。
///
/// 详见 https://developers.openai.com/codex/security。
///
/// 为某个 thread 加载 config 栈时，应关联一个 `cwd`（即 `cwd` 为 `Some(...)`）。
/// 仅在 thread 无关的配置加载场景（如 app server 的 `/config` 端点）下，
/// `cwd` 才应为 `None`。
///
/// # Errors
/// - 任一层的 TOML 解析或反序列化失败。
/// - requirements 层组合失败（如 hook 目录冲突）。
/// - 严格模式下未知字段校验失败。
/// - cloud config bundle 加载失败。
/// - thread config 加载失败。
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        ...
        // 出于向后兼容，继续支持 legacy `managed_config.toml` 位置作为
        // requirements 层加载。
    ...
    // 为"system" config 目录加入一个条目，若其 config.toml 存在则加载。
    ...
    // 添加基础 user config 层。当选择了 profile-v2 时，把 profile config 作为
    // 第二个 user 层叠加在上方，这样 profile 只需包含覆盖项。
    ...
    // 若存在来自 CLI 或 UI 的 runtime overrides，则添加对应层。
    ...
    // 尽力把 legacy `managed_config.toml` 作为最顶层 config 层支持。
    // 对于 `managed_config.toml` 中存在但 `ConfigRequirements` 没有对应字段的
    // 配置项，用户仍可在 TUI 与 VS Code 中按 turn 覆盖这些值。
    ...
    if let Some(config) = managed_config_from_mdm {
        // 一般规则：来自 MDM 的配置不应包含以 `./` 开头的相对路径，
        // 但以 `~/` 开头的路径是受支持的用例。由于
        // `resolve_relative_paths_in_config_toml()` 依赖
        // `AbsolutePathBufGuard` 解析 `~/`，必须为 `base_dir` 提供一个值。
        // 把同一个 base 保留到层上，以便后续原始 TOML 诊断使用相同的路径语义解析。
...
/// 加载 user config 层（可选 profile）。
///
/// `ignore_user_config` 为 `true` 时返回空表条目，不读取文件。否则调用
/// `load_config_toml_for_required_layer` 读取并解析 user_file。
///
/// # Errors
/// - 文件读取失败（且非 `NotFound`）。
/// - TOML 解析失败。
...
/// 按优先级把 `layer` 插入到 `layers` 中合适的位置。
///
/// 找到第一个优先级高于 `layer` 的既有层，在其前面插入；若没有则追加到末尾。
/// 这样保持 `layers` 始终按优先级升序排列。
...
/// 尝试从 `config_toml` 加载 config.toml 文件。
/// - 若文件存在且为合法 TOML，把解析后的 `toml::Value` 传给 `create_entry`
///   并返回构造的层条目。
/// - 若文件不存在，使用空 `Table` 调用 `create_entry` 并返回。
/// - 若读取或解析出错，返回错误。
///
/// # Errors
/// - 文件读取失败（且非 `NotFound`）。
...
///
/// 使用 `AbsolutePathBufGuard` 设置 `base_dir`，使相对路径校验与
/// 正常加载使用相同的路径语义。
///
/// # Errors
...
/// 严格模式下校验 CLI overrides 是否包含未知字段。
///
/// 同时检查顶层未知字段与 `features.*` 下的未知 feature 字段。
///
/// # Errors
/// 存在未知字段或未知 feature 时返回 `InvalidData` 错误。
...
/// 若可用，从平台系统级 `requirements.toml` 位置加载 requirements 层。
///
/// # Errors
/// - 文件读取失败（且非 `NotFound`）。
...
/// 返回 Unix 平台系统级 `requirements.toml` 的路径。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
#[cfg(unix)]
...
/// 返回 Windows 平台系统级 `requirements.toml` 的路径。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
#[cfg(windows)]
...
/// 根据 `overrides` 返回系统级 `requirements.toml` 的路径，允许覆盖。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
...
/// 返回 Windows 平台系统级 `config.toml` 的路径。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
#[cfg(windows)]
...
/// 根据 `overrides` 返回系统级 `config.toml` 的路径，允许覆盖。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
...
/// 返回 Windows 平台 Codex 系统目录（`<ProgramData>\OpenAI\Codex`）。
#[cfg(windows)]
...
/// 返回 Windows 平台系统级 `requirements.toml` 的完整路径。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
#[cfg(windows)]
...
/// 返回 Windows 平台系统级 `config.toml` 的完整路径。
///
/// # Errors
/// 路径转 `AbsolutePathBuf` 失败时返回错误。
#[cfg(windows)]
...
/// 通过 Windows Shell API 的 `SHGetKnownFolderPath` 解析 `ProgramData`
/// 已知文件夹（known folder）路径。
///
/// 失败时返回错误，调用方可回退到 [`DEFAULT_PROGRAM_DATA_DIR_WINDOWS`]。
///
/// # Errors
/// - `KF_FLAG_DEFAULT` 无法转换为 `u32`。
/// - `SHGetKnownFolderPath` 调用失败（HRESULT 非 0）。
/// - 返回的指针为 null。
///
/// # Safety
/// 调用 `SHGetKnownFolderPath` 与 `CoTaskMemFree` 涉及 FFI。前者在成功时
/// 通过 `path_ptr` 返回一个 CoTaskMem 分配的、null 结尾的 UTF-16 字符串，
/// 必须用 `CoTaskMemFree` 释放。
#[cfg(windows)]
fn windows_program_data_dir_from_known_folder() -> io::Result<PathBuf> {
    ...
    // SAFETY: SHGetKnownFolderPath 在成功时通过 path_ptr 初始化一个
    // CoTaskMem 分配的、null 结尾的 UTF-16 字符串。
    let hr = unsafe {
    ...
    // SAFETY: path_ptr 是由 SHGetKnownFolderPath 分配的合法 null 结尾
    // UTF-16 字符串，必须用 CoTaskMemFree 释放。
    let path = unsafe {
        let mut len = 0usize;
...
/// 把已加载的 legacy `managed_config.toml` 层转换为 requirements 层。
///
/// # Errors
/// legacy config 反序列化为 `LegacyManagedConfigToml` 失败时返回错误。
fn requirements_layers_from_legacy_scheme(
    loaded_config_layers: LoadedConfigLayers,
) -> io::Result<Vec<RequirementsLayerEntry>> {
    // 先列出文件来源的 legacy 层，因为 requirements 层按低优先级到高优先级
    // 顺序组合，而 MDM 的优先级高于 legacy managed_config.toml 文件。
    let LoadedConfigLayers {
        managed_config,
        managed_config_from_mdm,
    } = loaded_config_layers;
...
/// 把 `LegacyManagedConfigToml` 转换为 requirements TOML 值。
///
/// 映射规则：
/// - `approval_policy` → `allowed_approval_policies: [policy]`
/// - `approvals_reviewer` → `allowed_approvals_reviewers: [reviewer, ...]`
///   （`auto_review` 时额外允许 `user`，让用户可以退出 auto-reviewer）
/// - `sandbox_mode` → `allowed_sandbox_modes: [read-only, mode]`
///   （始终追加 `read-only`，因为只读是 Codex 正常运行的必要条件）
///
/// # Errors
/// 序列化为 `TomlValue` 失败时返回错误。
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // 允许只读是 Codex 正确运行的必要条件，因此在这个回填路径中，
        // 若 read-only 尚未被指定，则追加它。
...
impl ProjectTrustContext {
    ...
    /// 1. 该目录自身的规范化信任键。
    /// 2. 项目根的候选信任键。
    /// 3. repo root 的候选信任键（若与项目根不同）。
    ///
    /// 若都未命中，返回 `trust_level: None` 与 repo root 或项目根的键。
    ...
    /// 根据信任决策返回禁用原因消息（若不受信任）。
    ///
    /// 受信任时返回 `None`。未受信任时根据 `trust_level` 是 `Untrusted`
    /// 还是 `None` 返回不同的提示消息，告知用户如何在 user config 中
    /// 标记项目为受信任以加载 project-local config、hooks 与 exec policies。
    ...
    /// 对于 linked worktree，返回 repo root 下对应目录的 `.codex` 文件夹。
    ///
    /// 当 `checkout_root` 与 `repo_root` 相同时（普通 checkout），返回 `None`。
    /// 当二者不同时（linked worktree），把 `dir` 相对 `checkout_root` 的路径
    /// 拼接到 `repo_root` 下，并追加 `.codex`，以便从主仓库根读取 hook 配置。
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        // 普通 checkout 的两个路径解析到同一个根；linked worktree 则不同。
...
/// 构造一个 project config 层条目。
///
/// `disabled_reason` 为 `Some(_)` 时构造一个被禁用的层（携带禁用原因）。
/// `hooks_config_folder_override` 用于 linked worktree 场景下覆盖 hook 配置目录。
...
/// 从 project config 中移除项目级禁止的字段，返回被移除的键列表。
///
/// 移除 [`PROJECT_LOCAL_CONFIG_DENYLIST`] 中的顶层键，以及
...
/// 解析项目信任上下文。
///
/// 从合并后的 config 中反序列化 `[projects]` 表，查找项目根、checkout root
/// 与 repo root，构造 `ProjectTrustContext` 供后续 `load_project_layers` 使用。
///
/// # Errors
/// - `[projects]` 表反序列化失败。
/// - 项目根查找失败。
/// - 路径转 `AbsolutePathBuf` 失败。
...
/// 把路径规范化为项目信任映射的键字符串。
///
/// 在 Windows 上会转换为小写，以尽量保证指向同一位置的不同路径
/// （如带 UNC 前缀与不带）能映射到同一个键。同时返回 canonical 路径
/// 与原始路径的规范化结果（若不同）。
...
/// 返回路径的所有候选信任键（canonical 与原始，若不同则两个都返回）。
...
/// 规范化单个信任键：Windows 上转小写，其它平台原样返回。
...
/// 在 `projects_trust` 映射中按 `lookup_key` 查找信任级别。
///
/// 先精确匹配，若未命中再做一次规范化比较（用于 Windows 大小写不敏感匹配）。
/// 多个规范化匹配时按键排序取第一个，保证结果确定性。
...
/// 遍历从 config.toml 解析出的 `toml::Value`，把所有 `AbsolutePathBuf` 字段
/// 相对 `base_dir` 解析，返回形状相同但路径已解析的新 `toml::Value`。
///
/// 这确保从不同目录加载的多个 config 层可以正确合并。
#[doc(hidden)]
pub fn resolve_relative_paths_in_config_toml(
    value_from_config_toml: TomlValue,
    base_dir: &Path,
) -> io::Result<TomlValue> {
    // 通过序列化/反序列化往返，把 `toml::Value` 转换为 `ConfigToml`，
    // 在此过程中 `AbsolutePathBuf` 字段会相对 base_dir 解析。
    let _guard = AbsolutePathBufGuard::new(base_dir);
...
/// 确保 `original` 中的每个字段在返回的 `toml::Value` 中都存在，
/// 尽可能取 `resolved` 的值。这是为了保留在 `resolve_relative_paths_in_config_toml`
/// 的序列化/反序列化往返中被"丢弃"的字段，出于谨慎而保留原始形状。
...
/// 从 `cwd` 向上查找项目根目录。
///
/// 遍历 `cwd` 的所有祖先目录，找到第一个包含任一 `project_root_markers`
/// 标记文件的目录作为项目根。若未找到或 markers 为空，返回 `cwd` 本身。
///
/// # Errors
/// 一般不会返回错误（仅返回 `Ok(cwd)` 或 `Ok(ancestor)`）。
...
/// 从 `cwd` 向上查找 git checkout root（即包含 `.git` 的目录）。
///
/// 返回 `None` 表示未找到 `.git`。
...
struct LoadedProjectLayers {
    /// 项目 config 层列表（按优先级升序）。
    layers: Vec<ConfigLayerEntry>,
    /// 启动警告消息（如被忽略的项目级配置键）。
    startup_warnings: Vec<String>,
}

/// 加载 `cwd` 到 `project_root` 之间（含两端）的所有项目 config 层。
///
/// 返回的层列表按优先级升序排列：最靠近 `project_root` 的目录优先级最低，
/// 最靠近 `cwd` 的目录优先级最高。`startup_warnings` 是栈级启动消息，
/// 不作为额外的 config 层。
///
/// # Errors
/// - 受信任目录的 config.toml 解析失败。
/// - 严格模式下未知字段校验失败。
/// - 相对路径解析失败。
/// - 文件读取失败（且非 `NotFound`）。
        ...
        match fs.read_file_text(&config_file_uri, /*sandbox*/ None).await {
            ...
            Err(err) => {
                if err.kind() == io::ErrorKind::NotFound {
                    // 若不存在 config.toml 文件，仍为该项目层记录一个空条目，
                    // 因为它可能仍包含在整体 ConfigLayerStack 中有意义的子目录。
                    let config = merge_root_checkout_project_hooks(
                        fs,
                        TomlValue::Table(toml::map::Map::new()),
                        hooks_config_folder_override.as_ref(),
...
/// 对于 linked worktree，保留普通 worktree 本地的项目配置，仅把 hook 声明
/// 替换为匹配的 root-checkout 层。
///
/// # Errors
/// - 受信任目录的 hook config 解析失败。
/// - 相对路径解析失败。
/// - 文件读取失败（且非 `NotFound`）。
...
/// legacy 的 admin 强制配置机制：从 `/etc/codex/managed_config.toml` 等
/// 文件读取与 `config.toml` 结构相同的内容，但 `approval_policy` 等字段
/// 只能指定一个值而非允许值列表。
///
/// 若存在，则把 `managed_config.toml` 重新解释为 `requirements.toml`，
/// 其中每个指定的字段被视为约束。大多数字段只允许指定的值。
/// `approvals_reviewer = "auto_review"` 还额外允许 `user`，以便用户退出
/// auto-reviewer。
#[derive(Deserialize, Debug, Clone, Default, PartialEq)]
struct LegacyManagedConfigToml {
    /// legacy `approval_policy` 字段。
    approval_policy: Option<AskForApproval>,
    /// legacy `approvals_reviewer` 字段。
    approvals_reviewer: Option<ApprovalsReviewer>,
    /// legacy `sandbox_mode` 字段。
    sandbox_mode: Option<SandboxMode>,
}

// 不能命名为 `mod tests`，因为本目录下已存在 tests.rs 文件。
#[cfg(test)]
mod unit_tests {
    use super::*;
    #[cfg(windows)]
