//! 配置解析/校验错误的诊断与渲染。
//!
//! 把 TOML 解析与 schema 校验失败映射到具体文件位置（1-based 行列），
//! 并渲染为带 `^` 高亮的可读错误信息。支持从多层 layer 栈中定位第一个
//! 出错的文件，便于在合并后的配置校验失败时指向具体来源 layer。
...
/// 配置加载错误：包装 `ConfigError` 与可选的底层 `toml::de::Error`。
///
/// 实现 `std::error::Error::source()` 以暴露底层 toml 解析错误。
#[derive(Debug)]
...
impl ConfigDiagnosticSource<'_> {
    /// 转换为 `PathBuf`，用于填充 `ConfigError.path`。
...
/// 把 `ConfigError`（含可选底层 toml 错误）包装为 `io::Error`。
...
/// 由 toml 解析错误构造 `ConfigError`，定位到出错文件与字节范围。
...
/// 由 toml 解析错误与诊断来源构造 `ConfigError`（内部实现）。
...
/// 尝试将 TOML 文本反序列化为 `T`，失败时返回首个 `ConfigError`。
///
/// 使用 `serde_path_to_error` 定位到出错字段路径，再通过 `toml_edit`
/// 把路径映射回文件中的字节范围。解析成功返回 `None`。
...
/// 遍历 layer 栈（低优先级在前），返回第一个出错的 layer 的 `ConfigError`。
///
/// 当合并后的配置 schema 校验失败时，定位到具体出错的文件与范围，而非
/// 暴露合并后的不可读错误。
pub async fn first_layer_config_error<T: DeserializeOwned>(
    layers: &ConfigLayerStack,
    config_toml_file: &str,
) -> Option<ConfigError> {
    // 合并配置校验失败时，定位到第一个具体出错的文件与范围，
    // 而非返回合并后的不可读错误。
...
/// 与 `first_layer_config_error` 类似，但接受已展开的 layer 切片。
    ...
    I: IntoIterator<Item = &'a ConfigLayerEntry>,
{
    for layer in layers {
        if let Some(contents) = layer.raw_toml() {
            ...
            // 与原始非文件 layer 解析为运行时 layer 时使用相同 base 目录，
            // 确保相对路径字段的诊断语义一致。
...
/// 把字节范围转换为 1-based 行列 `TextRange`。
///
/// `end` 取 `span.end - 1`（当 span 非空时）以指向最后一个字符而非下一
/// 个字符的开头。
...
/// 读取 `error.path` 文件内容并调用 `format_config_error` 渲染。
///
/// 读文件失败时以空内容渲染（仅输出位置与消息，无源代码高亮）。
