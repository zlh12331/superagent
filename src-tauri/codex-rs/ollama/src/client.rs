//! Ollama HTTP 客户端模块。
//!
//! 封装与本地 Ollama 服务的所有 HTTP 交互：探测可达性、查询本地模型、
//! 拉取模型并解析流式事件。同时支持 Ollama 原生 API 与 OpenAI 兼容端点。
...
/// 当未检测到运行中的 Ollama 服务时返回的错误信息（含安装/启动指引）。
...
///
/// 字段含义：
/// - `client`：底层 reqwest 客户端
/// - `host_root`：Ollama 服务根 URL（去除末尾 `/v1` 后的形态）
/// - `uses_openai_compat`：是否使用 OpenAI 兼容端点（base_url 以 `/v1` 结尾）
...
impl OllamaClient {
    /// 基于内置开源（"oss"）模型 provider 构造客户端，并验证本地 Ollama 服务可达。
    ///
    /// 若未检测到服务，返回包含安装/启动指引的错误。
    pub async fn try_from_oss_provider(config: &Config) -> io::Result<Self> {
        // 必须从 Config 中查找 provider，确保用户在 config.toml 中的覆盖配置生效
        let provider = config
            .model_providers
    ...
    pub async fn pull_with_reporter(
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                ...
                PullEvent::Error(err) => {
                    // 经验上 ollama 即便输出流包含错误消息也返回 200 OK，可用以下命令验证：
                    //
                    // `curl -i http://localhost:11434/api/pull -d '{ "model": "foobarbaz" }'`
                    //
                    // 因此必须检查事件流而非 HTTP 响应状态来决定是否返回 Err
    ...
    /// 低层构造器：直接传入 host root（如 "http://localhost:11434"）。
    #[cfg(test)]
