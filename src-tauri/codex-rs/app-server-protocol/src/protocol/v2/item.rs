...
pub enum ThreadItem {
    ...
    CommandExecution {
        ...
        /// 对 command 进行 best-effort 解析，以理解它将执行的动作（action）。
        /// 该字段返回一个 CommandAction 对象列表，因为单个 shell command 可能
        /// 由多个通过管道连接的命令组合而成。
        command_actions: Vec<CommandAction>,
        /// command 的输出，从 stdout 和 stderr 聚合而来。
        aggregated_output: Option<String>,
        /// command 的退出码。
        exit_code: Option<i32>,
        /// command 执行的 duration，单位为 milliseconds。
        #[ts(type = "number | null")]
    ...
    McpToolCall {
        ...
        /// MCP tool call 的 duration，单位为 milliseconds。
        #[ts(type = "number | null")]
        duration_ms: Option<i64>,
    },
    #[serde(rename_all = "camelCase")]
    ...
    DynamicToolCall {
        ...
        success: Option<bool>,
        /// dynamic tool call 的执行时长，单位为毫秒（ms）。
        /// 通常在调用结束（成功或失败）后由服务端填充；进行中或未触发结束时为 `None`。
        #[ts(type = "number | null")]
        duration_ms: Option<i64>,
    },
    #[serde(rename_all = "camelCase")]
...
/// 已废弃（deprecated）的 legacy notification，用于传递 `apply_patch` 的文本输出。
///
/// 服务端不再发出此 notification，结构保留仅为兼容旧客户端读取历史数据。
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
pub struct CommandExecutionRequestApprovalParams {
    ...
    /// 待执行的命令（command）字符串。
    /// 在 exec command 审批请求场景下，此字段携带客户端需要审批的具体命令内容。
    #[serde(default, skip_serializing_if = "Option::is_none")]
    #[ts(optional = nullable)]
