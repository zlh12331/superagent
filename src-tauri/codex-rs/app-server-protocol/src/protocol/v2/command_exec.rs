...
/// 在服务端 sandbox 中执行一条独立的命令（以 argv vector 形式提供），
/// 此过程不会创建 thread 或 turn。
///
/// 最终的 `command/exec` 响应会被推迟到进程退出之后才返回，并且保证在
/// 该连接所有 `command/exec/outputDelta` notification 都发出之后才发送，
/// 以确保客户端能按顺序收到完整的输出流。
