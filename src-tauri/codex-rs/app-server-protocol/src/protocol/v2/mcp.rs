...
pub struct McpServerElicitationRequestParams {
    pub thread_id: String,
    /// 观察到该 elicitation 时处于活跃状态的 Codex turn（如果 app-server 能关联到）。
    ///
    /// 该字段是 nullable 的，因为 MCP 将 elicitation 建模为 standalone server-to-client request，
    /// 由 MCP server 的 request id 标识。它可能在某个 turn 期间被触发，但 turn 上下文
    /// 属于 app-server 的关联信息，而非 elicitation 自身协议身份的一部分。
...
/// MCP `elicitation/create` 请求的类型化 form schema。
///
/// 该 schema 与 MCP 2025-11-25 `ElicitRequestFormParams`
/// schema 中的 `requestedSchema` shape 一致。
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
pub struct McpServerElicitationRequestResponse {
    ...
    ///
    /// 该字段是 nullable 的，因为 decline/cancel responses 没有 content。
