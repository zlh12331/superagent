...
/// 该 translation layer 用于确保以 camel case 暴露 codex 错误码。
///
/// 当存在 upstream HTTP status 时（例如来自 Responses API 或 provider），
/// 它会通过相应 `codexErrorInfo` variant 的 `httpStatusCode` 字段转发。
