...
pub struct PluginInstalledParams {
    ...
    #[ts(optional = nullable)]
    pub cwds: Option<Vec<AbsolutePathBuf>>,
    /// 额外的、尚未安装的 plugin 名称；当本地存在时应一并返回。
    /// 该字段供 mention surfaces 使用，这些 surface 会刻意暴露 install entrypoints。
    #[ts(optional = nullable)]
...
pub enum PluginSource {
    ...
    /// 该 plugin 在 remote catalog 中可用。Download metadata 保留在服务端，
    /// 不通过 app-server API 暴露。
    Remote,
}
