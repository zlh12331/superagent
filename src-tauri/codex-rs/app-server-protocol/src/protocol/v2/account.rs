...
pub enum LoginAccountParams {
    ...
    ChatgptDeviceCode,
    /// [不稳定接口（UNSTABLE）] 仅供 OpenAI 内部使用，请勿外部使用。
    /// 所提供的 access token 必须包含与 Codex 托管的 ChatGPT auth token 相同的
    /// scopes（权限范围），否则后端 API 调用会被拒绝。
    #[experimental("account/login/start.chatgptAuthTokens")]
    #[serde(rename = "chatgptAuthTokens", rename_all = "camelCase")]
    #[ts(rename = "chatgptAuthTokens", rename_all = "camelCase")]
    ChatgptAuthTokens {
        /// 由客户端提供的 access token（JWT 格式）。
        /// 该 token 用于发起 backend API 请求以及从 token claims 中提取用户邮箱
        /// （email extraction）。
        access_token: String,
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    ///
    /// 当先前的 auth 状态未包含 workspace 标识符（`chatgpt_account_id`）时，
    /// 此字段可能为 `null`；调用方需对 `None` 情况进行处理。
    #[ts(optional = nullable)]
...
pub enum ConsumeAccountRateLimitResetCreditOutcome {
    ...
    NothingToReset,
    /// 当前 Account 已无可用的（earned）reset credits，无法执行重置操作。
    NoCredit,
    /// 同一 idempotency key 此前已成功完成过一次重置，本次请求视为重复消费（already redeemed）。
    AlreadyRedeemed,
}
