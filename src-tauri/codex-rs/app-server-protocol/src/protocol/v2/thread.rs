...
pub struct ThreadStartParams {
    ...
    /// 若为 true，则选择在事件流上发射原始 Responses API item。
    /// 该选项仅供 internal use only（例如 Codex Cloud）。
...
pub struct ThreadResumeInitialTurnsPageParams {
    ...
    /// 为每个返回的 turn 包含多少 item detail；默认为 summary。
    #[ts(optional = nullable)]
...
pub struct ThreadForkParams {
    ...
    ///
    /// 指定后，`last_turn_id` 之后的 turn 会从 fork 中省略。
    /// 被引用的 turn 不能处于 in progress 状态。
    #[ts(optional = nullable)]
...
pub struct ThreadRollbackParams {
    pub thread_id: String,
    /// 要从 thread 末尾丢弃的 turn 数量。必须 >= 1。
    ///
    /// 该操作仅修改 thread 的历史记录，不会回退 agent 已做出的本地文件更改。
    /// 客户端负责自行回退这些更改。
    pub num_turns: u32,
}
...
pub struct ThreadRollbackResponse {
    /// 应用 rollback 后的已更新 thread，其中 `turns` 已填充。
    ///
    /// 每个 Turn 中存储的 ThreadItems 是有损的，因为我们显式地不持久化
    /// 所有 agent 交互（例如命令执行）。这与 `thread/resume` 的行为一致。
    pub thread: Thread,
}
...
pub struct ThreadListResponse {
    ...
    /// if None, there are no more items to return.
    pub next_cursor: Option<String>,
    /// 在反转 `sortDirection` 时作为 `cursor` 传入的不透明 cursor。
    /// 仅当 page 至少包含一个 thread 时才会填充该字段。
    /// 需配合相反的 `sortDirection` 使用；对于按时间戳排序的情况，
    /// 它会锚定在 page 时间戳的起始处，以避免跳过同秒更新。
    pub backwards_cursor: Option<String>,
}
...
pub struct ThreadTurnsListResponse {
    ...
    pub next_cursor: Option<String>,
    /// 在反转 `sortDirection` 时作为 `cursor` 传入的不透明 cursor。
    /// 仅当 page 至少包含一个 turn 时才会填充该字段。
    /// 需配合相反的 `sortDirection` 使用，以再次包含锚点 turn
    /// 并捕获对该 turn 的更新。
    pub backwards_cursor: Option<String>,
}
