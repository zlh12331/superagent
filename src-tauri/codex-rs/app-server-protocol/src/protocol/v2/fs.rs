...
/// 在宿主文件系统（host filesystem）上创建目录。
/// 对应 `fs/createDirectory` 请求，用于按指定路径创建新目录。
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// 针对一个绝对路径启动文件系统监听（filesystem watch）。
/// 注册成功后，当该路径下的文件或目录发生变更时，服务端会通过 `fs/changed`
/// notification 主动推送给客户端。
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
...
/// 停止此前由 `fs/watch` 注册的文件系统监听。
/// 客户端断开连接时，与之关联的 watch 也会自动失效。
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
