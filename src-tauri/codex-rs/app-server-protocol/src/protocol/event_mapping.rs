...
/// 根据单条 core event 构建与之直接对应的 v2 app-server notification。
///
/// 此处仅覆盖那些具有一一对应关系的 stateless event-to-notification projection
/// （无状态事件到通知的映射）。调用方在调用此 helper 之前，仍需自行负责必要的
/// 前置状态检查与副作用处理。
