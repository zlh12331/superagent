...
impl ThreadHistoryBuilder {
    ...
    /// 返回当前活动 turn 在已完成 turn 列表中所处的 index。
    ///
    /// 当某个 turn 仍处于 open 状态时，返回值是它经过 `finish` 后将占据的位置；
    /// 当没有 open 的 turn 时，返回值是最后一个已完成 turn 的 index。
    ...
    /// 共享的 reducer，既用于 persisted rollout 的回放，也用于运行中 thread
    /// resume/rejoin 时对 current-turn 的内存跟踪。
    ///
    /// 此函数应当处理所有能够被持久化到 rollout 文件中的 EventMsg 变体，
    /// 详见 `codex-rs/core/rollout/policy.rs` 中的 `should_persist_event_msg`。
    ...
    /// 将当前 turn 标记为包含已持久化的 compaction marker。
    ///
    /// 这样可以避免那些仅包含 compaction 的 legacy turn 在没有任何可渲染 item
    /// 且未被显式 open 的情况下，被 `finish_current_turn` 误丢弃。
