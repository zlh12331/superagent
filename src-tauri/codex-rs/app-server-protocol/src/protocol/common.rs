...
client_request_definitions! {
    ...
    /// 递增 thread 本地的 out-of-band elicitation 计数器。
    ///
    /// 该方法供 external helpers 使用，用于在 user approval 或其他 elicitation
    /// 于 app-server 请求流之外挂起时暂停 timeout accounting。
    ...
    /// 自减 thread-local 的 out-of-band elicitation 计数器。
    ///
    /// 当计数归零时，该 thread 的 timeout 计时恢复进行。
...
server_request_definitions! {
    /// 新增 API
    /// 当需要对某条具体的命令执行（command execution）请求审批时发送。
    /// 此 request 用于通过 turn/start 启动的 Turns。
    ...
    /// 以下为已废弃（DEPRECATED）的 API
    /// 请求审批一个 patch。
    /// 此 request 用于通过 legacy API（即 SendUserTurn、SendUserMessage）启动的 Turns。
    ApplyPatchApproval {
        params: v1::ApplyPatchApprovalParams,
        response: v1::ApplyPatchApprovalResponse,
    },
    /// 请求执行一个 command。
    /// 此 request 用于通过 legacy APIs（即 SendUserTurn、SendUserMessage）启动的 Turns。
    ExecCommandApproval {
...
server_notification_definitions! {
    ...
    /// 此 event 仅限内部使用（internal-only），由 Codex Cloud 使用。
...
mod tests {
    ...
    fn serialize_client_notification() -> Result<()> {
        ...
        // 注意：此 notification 不含 "params" 字段。
        assert_eq!(
            json!({
                "method": "initialized",
            }),
