...
/// 从 mixed export 中构建一份适配 datamodel-code-generator 的 v2 schema bundle。
///
/// 完整的 bundle 会将 v2 schemas 嵌套在 `definitions.v2` 之下，同时保留少量
/// 共享的根级 definitions（例如 `ClientRequest` 与 `ServerNotification`）。
/// Python codegen 仅会遍历一层 definitions map，因此若直接喂入原始 bundle，
/// 会把 `v2` 自身误认作一个 schema，从而漏掉那些未被引用的 v2 叶子节点。
/// 此辅助函数将所有 v2 definitions 拍平到根级 definitions map，
/// 再将共享的根级 schemas 以及它们仍然引用的非 v2 传递依赖一并拉入。
/// 注意：此处必须保留共享的根级 union 不动——部分合法的
/// request/notification/event 变体是内联定义或仅引用共享的根级 helper，
/// 若按是否包含 `#/definitions/v2/` ref 来过滤，会从 flat bundle 中
/// 静默丢弃真实的 API 表面（API surface）。
...
/// 递归地将裸的根级 definition ref 重写为 bundle 中实际拥有该类型的 namespace。
///
/// mixed export 中包含若干有意保留在 `v2` namespace 之外的共享根级 helper
/// schemas，但其提取出的部分子 definitions 仍带有形如
/// `#/definitions/ThreadId` 的 ref。当真实 schema 仅存在于
/// `#/definitions/v2/ThreadId` 之下时，这些 ref 会变成悬空引用（dangling
/// ref），下游 codegen 会回退到占位用的 `Any` 模型。此重写操作将共享 helper
/// 保留在根级，同时把它们的 ref 重新指向实际存在的 namespaced definitions。
...
/// 生成一个 index.ts 文件，用于 re-export 所有已生成的类型。
/// 这样 consumer 只需从单一文件即可导入全部类型，便于使用。
