...
/// 从 `codex_core::config::Config` 派生的 MCP runtime settings。
///
/// 此 struct 应仅包含 `codex-mcp` crate 构造 server transports、
/// 执行 MCP approval/sandbox policy、定位 OAuth state 以及合并
/// plugin-provided MCP servers 所需的长期存活配置值。Request-scoped 或
/// auth-scoped 的状态不应存储在此处；应将这些值显式地传入 runtime entry points，
/// 例如 [`effective_mcp_servers`] 和 snapshot collection helpers，
/// 以便在 auth 变更时 config objects 不会过期。
#[derive(Debug, Clone)]
