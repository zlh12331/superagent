//! Model Context Protocol (MCP) servers 的 Runtime 支持。
//!
//! 本模块包含描述 MCP servers 执行的 runtime environment 的数据，
//! 以及发送给 capable servers 的 sandbox state payload 和一个小型共享
//! metrics 辅助工具。Transport startup 和 orchestration 位于
//! [`crate::rmcp_client`] 和 [`crate::connection_manager`]。
