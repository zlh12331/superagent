//! Windows sandbox 模块。
//!
//! 提供 Windows 平台下沙箱等级(`WindowsSandboxLevel`)的解析与配置编辑能力,
//! 包括从 config / features 推导沙箱等级、构造 NUX(初次体验)流程所需的
//! 配置编辑项,以及 metrics tag 的清洗。
//!
//! # 与其他模块的关系
//! - 被 [`crate::sandboxing`] 在 Windows 平台调用;
//! - 读取 [`crate::config::Config`] 的 `permissions.windows_sandbox_mode`
//!   与 features 配置;
//! - 产出 [`WindowsSandboxLevel`] 供 [`crate::sandbox_tags`] 等模块使用。
