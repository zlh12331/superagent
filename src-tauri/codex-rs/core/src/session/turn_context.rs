//! Turn context 模块:单次 turn 的不可变上下文。
//!
//! 定义 [`TurnContext`] 与 [`TurnEnvironment`] 等类型,封装一次 turn
//! 执行期间所需的全部不可变信息:配置、模型信息、权限、环境、
//! reasoning effort、collaboration mode 等。
//!
//! `TurnContext` 在 turn 启动时构造,在整个 turn 生命周期内不可变;
//! 若需要切换模型,则通过 [`TurnContext::with_model`] 派生新的 context。
...
/// 单次 turn 所需的不可变上下文。
///
/// 在 turn 启动时构造,封装配置、模型信息、权限、环境、reasoning effort 等。
/// 整个 turn 生命周期内不可变;需要切换模型时通过 [`Self::with_model`] 派生。
#[derive(Debug)]
pub struct TurnContext {
    ...
    /// session 的绝对工作目录。模型提供的所有相对路径以及 sandbox policy
    /// 都基于此路径解析,而非 `std::env::current_dir()`。
