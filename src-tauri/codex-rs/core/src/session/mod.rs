//! Session 模块:codex 单次会话的核心状态机。
//!
//! 维护一次 codex 会话(`Session`)的全部状态:配置、history、rollout、
//! MCP 连接、审批流程、guardian review、subagent 编排等。
//!
//! # 主要子模块
//! - [`session`]:核心 `Session` 结构体与生命周期管理;
//! - [`turn`]:turn 执行循环;
//! - [`turn_context`]:单次 turn 的不可变上下文;
//! - [`handlers`]:turn item 处理器;
//! - [`inject`]:context 注入;
//! - [`mcp`]:MCP 工具调用集成;
//! - [`review`]:审批请求流程。
//!
//! # 与其他模块的关系
//! - [`crate::codex_thread`] 持有 `Session` 并驱动 turn 循环;
//! - [`crate::client`] 通过 `Session` 与模型 API 通信;
//! - [`crate::guardian`] 在审批流程中被调用。
