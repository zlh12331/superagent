//! Thread manager 模块。
//!
//! 负责管理 codex thread 的完整生命周期,包括:
//! - 根据配置创建 [`CodexThread`] 与底层 [`Codex`] session;
//! - 装配所需的依赖(auth、MCP、agent graph、plugins、skills、
//!   time provider、attestation provider 等);
//! - 提供 thread store 的构造入口;
//! - 处理 multi-agent 协作模式与 collaboration mask;
//! - 关联 models manager 与 model provider。
//!
//! # 与其他模块的关系
//! - 顶层入口,由 app-server / CLI 调用;
//! - 内部组合 [`crate::session`]、[`crate::agent`]、[`crate::mcp`]、
//!   [`crate::rollout`] 等多个子系统。
