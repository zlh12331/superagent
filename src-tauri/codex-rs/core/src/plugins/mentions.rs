//! 插件与工具的 @提及（mention）解析逻辑。
//!
//! 本模块从用户输入（结构化 Mention 项或文本中的 `@`/`$` 前缀）
//! 中提取显式提及的 app、plugin 与 tool，供上下文注入使用。
...
/// 收集用户输入中显式提及的 app ID 集合。
///
/// 同时处理结构化 `UserInput::Mention` 与文本中的 `$app://...` 提及。
...
/// 收集显式提及的插件（结构化或 `plugin://...` 链接）。
///
/// 返回与 `plugins` 列表中 `config_name` 匹配的插件能力摘要。
...
/// 统计每个 connector 的 mention slug 出现次数。
