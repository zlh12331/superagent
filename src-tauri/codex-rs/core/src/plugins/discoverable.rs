//! 可发现插件（discoverable plugins）查询逻辑。
//!
//! 本模块封装从 PluginsManager 查询 tool_suggest 可发现插件的能力，
//! 将配置项与已加载的 connector ID 转换为查询输入，并映射为 `DiscoverablePluginInfo`。
...
/// 查询 tool_suggest 可发现插件列表。
///
/// 根据配置中的 discoverables 与 disabled_tools 过滤，结合已加载的 connector ID，
/// 调用 `PluginsManager` 获取可发现插件信息。
