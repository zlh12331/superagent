//! Linux Landlock 沙箱启动模块。
//!
//! 在 Linux 平台下通过独立的 `codex-linux-sandbox` 辅助可执行程序
//! 启动 shell 工具命令,默认使用 bubblewrap 进行文件系统隔离,
//! 并配合 seccomp 限制网络访问。
//!
//! 与 macOS Seatbelt 不同(直接内嵌 policy 文本),Linux 的沙箱策略
//! 通过 JSON 形式的 permission profile 传给辅助可执行程序,
//! 由该 helper 在运行时推导文件系统与网络策略。
//!
//! # 与其他模块的关系
//! - 被 [`crate::sandboxing`] 在 Linux 平台调用;
//! - 复用 [`crate::spawn`] 的子进程启动能力;
//! - 依赖 [`codex_sandboxing::landlock`] 提供的参数构造函数。
