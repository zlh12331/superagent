//! Responses 请求的重试与传输回退决策模块。
//!
//! 集中处理 Responses API 流式请求中可重试错误的逻辑,包括:
//! - 指数退避重试(超过最大重试次数后放弃);
//! - 当达到重试上限时,尝试从 WebSocket 传输回退到 HTTPS 传输;
//! - 向 session 发送 `Warning` 事件通知 UI 层。
//!
//! 该模块被 sampling 请求与 remote compaction v2 请求共用。
