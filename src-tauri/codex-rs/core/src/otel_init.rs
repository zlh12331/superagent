//! OpenTelemetry(OTel)初始化模块。
//!
//! 根据应用 [`crate::config::Config`] 构造 [`OtelProvider`],
//! 支持多种 exporter 类型(None / Statsig / OTLP HTTP / OTLP gRPC),
//! 并可配置 TLS 与自定义 headers。
//!
//! 当 OTEL 导出功能未启用或未配置时返回 `None`,由调用方决定是否降级。
//!
//! # 与其他模块的关系
//! - 读取 [`crate::config::Config`] 中的 `otel` 配置段;
//! - 产出的 [`OtelProvider`] 由 [`crate::codex_thread`] 等顶层运行时持有,
//!   用于后续 metrics / traces 上报。
