//! Guardian review 指标上报模块。
//!
//! 将每次 guardian review 的决策、耗时、token 使用量等指标
//! 通过 `SessionTelemetry` 上报至 OTel metrics 管线,
//! 供后续监控与分析。
...
/// 上报一次 guardian review 的完整指标。
///
/// 包括:
/// - review 计数(counter);
/// - review 总耗时(histogram);
/// - 首个 token 耗时(TTFT,histogram,若存在);
/// - token 使用量(histogram,按 token 类型分桶)。
///
/// # 参数
/// - `session_telemetry`:会话级 telemetry 上下文;
/// - `result`:guardian review 的分析结果;
/// - `approval_request_source`:审批请求来源(主 turn / 委派 subagent);
/// - `reviewed_action`:被审批的动作类型;
/// - `completion_latency_ms`:本次 review 的端到端耗时(毫秒)。
...
/// 上报 token 使用量的分桶直方图。
///
/// 将总 token、输入 token、缓存输入 token、非缓存输入 token、
/// 输出 token、推理输出 token 分别作为不同 `token_type` tag 上报。
...
/// 组装 guardian review 指标的 tag 列表。
///
/// 将决策、终态、失败原因、审批来源、动作类型、session kind、
/// 是否有前置 review 上下文、动作是否截断、风险等级、用户授权、
/// outcome、guardian 模型与推理 effort 等信息全部作为 tag 上报,
/// 便于在监控系统中按多维度聚合分析。
