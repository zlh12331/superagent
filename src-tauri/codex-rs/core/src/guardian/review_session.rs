//! Guardian review session 管理模块。
//!
//! 负责创建、复用与销毁 guardian review session,核心抽象是
//! [`GuardianReviewSessionManager`]。它维护一个 trunk session 用于
//! 跨多次 review 复用,以及在 trunk 被占用或配置变化时 spawn 临时
//! ephemeral session。
//!
//! # 复用策略
//! - 通过 [`GuardianReviewSessionReuseKey`] 判断配置是否一致;
//! - 配置一致时优先复用 trunk,使用 `review_lock` 串行化 review;
//! - trunk 被占用或配置变化时 fork 出 ephemeral session,用完即销毁。
//!
//! # 与其他模块的关系
//! - 由 [`super::review`] 在发起审批评估时调用;
//! - 通过 [`crate::codex_delegate`] spawn 子 codex 线程;
//! - 使用 [`super::prompt`] 组装 prompt items。
...
/// guardian review session 的最终执行结果。
///
/// 区分正常完成、prompt 构建失败、session 错误、超时与中止。
#[derive(Debug)]
pub(crate) enum GuardianReviewSessionOutcome {
    /// 正常完成,携带模型最终消息(可能为空)或错误。
    Completed(anyhow::Result<Option<String>>),
    /// prompt 构建失败。
    PromptBuildFailed(anyhow::Error),
    /// session 执行失败,携带错误信息与可选的结构化 codex error。
    SessionFailed {
        error: anyhow::Error,
...
/// 发起一次 guardian review 所需的全部参数。
///
/// 由调用方(通常是 [`super::review`])组装后传入
...
/// guardian review session 管理器。
///
/// 维护一个可复用的 trunk session 与一组 ephemeral session,
/// 通过 [`GuardianReviewSessionReuseKey`] 判断是否可复用。
#[derive(Default)]
...
/// 计算 guardian review session 的 prompt cache key 覆盖值。
///
/// 仅当 session 来源是 guardian reviewer 且存在 parent thread id 时,
/// 返回 `guardian:{parent_thread_id}` 作为 cache key,使同一父线程下的
/// 多次 guardian review 能共享 prompt cache 前缀。
...
impl GuardianReviewSessionManager {
    /// 预先初始化 trunk session,使首次 review 不必等待 spawn。
    ///
    /// 使用 `Box::pin` 打破 `Session::new -> Guardian -> Session::new`
    /// 的 future 递归;若在 spawn 期间已发生首次 review 或 shutdown,
    /// 则放弃本次初始化结果。
    ...
    /// 返回 trunk session 的 rollout 文件路径(用于 fork snapshot)。
    ///
    /// 若 trunk 不存在或路径解析失败,返回 `None`。
    ...
    /// 关闭所有 trunk 与 ephemeral session。
    ///
    /// 先取消 cancellation token,再依次关闭 trunk 与所有 ephemeral session。
    ...
    /// 执行一次 guardian review。
    ///
    /// 决策流程:
    /// 1. 若 trunk 的 reuse key 与本次配置不一致,则关闭旧 trunk 并 spawn 新 trunk;
    /// 2. 若 trunk 可用且未被占用(`review_lock` 可获取),则在 trunk 上执行 review;
    /// 3. 若 trunk 被占用或配置不一致,则 fork 出 ephemeral session 执行 review,
    ///    完成后立即销毁。
    ///
    /// 返回 review 的最终 outcome 与 analytics 结果。
    #[expect(
...
/// 基于父配置构建 guardian review session 的 spawn config。
///
/// 在父配置基础上做以下调整:
/// - 切换到指定的 guardian model 与 reasoning effort;
/// - 关闭 skill instructions、memories、apps instructions;
/// - 设置 base instructions 为 guardian policy prompt;
/// - 关闭 approval(设为 `Never`)与 MCP servers;
/// - 将 permission profile 设为只读;
/// - 禁用一系列与 review 无关的 feature(SpawnCsv、Collab、MultiAgentV2、
///   CodexHooks、Apps、Plugins、WebSearch 等);
/// - 在有 live network config 时,根据约束重新计算 network proxy 配置。
