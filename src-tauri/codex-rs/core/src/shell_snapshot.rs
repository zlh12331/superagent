//! Shell snapshot 模块。
//!
//! 负责在 turn 执行前捕获用户 shell 环境的快照(env / aliases / functions),
//! 以便在沙箱内还原用户的 shell 上下文。快照以文件形式持久化,
//! 关联到具体的 thread 与 session telemetry。
//!
//! # 与其他模块的关系
//! - 依赖 [`crate::shell`] 获取 shell 类型与执行能力;
//! - 通过 [`crate::StateDbHandle`] 关联持久化状态;
//! - 由 session 在需要时调用以准备执行环境。
