//! 网络代理配置加载与热重载。
//!
//! 本模块从 Codex 配置层（system/user/project）加载网络代理配置，
//! 合并 execpolicy 网络规则，构建 `NetworkProxyState`，
//! 并提供基于文件 mtime 的配置热重载器 `MtimeConfigReloader`。
...
/// 基于文件 mtime 的配置热重载器。
///
/// 定期检查各配置层的 mtime，若任一层文件发生变化则重新构建配置状态。
