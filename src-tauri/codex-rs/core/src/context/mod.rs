//! context 片段(fragment)注入模块。
//!
//! 集中维护所有需要注入到模型 input 中的 context 片段。
//!
//! # 设计概要
//! 每个片段对应一个具体场景(如 apps 指引、skills 可用性、guardian 复审提醒、
//! 时间提醒、网络规则、token budget 等),由 session 在 turn 启动时按需组装。
//!
//! # 与其他模块的关系
//! - [`crate::session`] 调用本模块组装 initial context;
//! - [`crate::compact`] 在压缩时通过本模块重建 initial context;
//! - [`crate::guardian`] 通过本模块触发 follow-up review 提醒。
