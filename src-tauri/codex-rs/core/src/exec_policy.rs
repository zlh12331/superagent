//! 执行策略(exec_policy)模块。
//!
//! 负责加载、解析与评估 execpolicy 规则文件,决定一条 shell 命令是否
//! 需要审批、是否应被拒绝,以及如何根据用户审批策略 [`AskForApproval`] 与
//! permission profile 应用规则。
//!
//! # 与其他模块的关系
//! - [`crate::exec`] 在执行命令前调用本模块进行策略评估;
//! - [`crate::tools::sandboxing`] 根据 [`ExecApprovalRequirement`] 决定是否触发用户审批;
//! - rules 文件存放在 `~/.codex/rules/` 目录,默认策略文件为 `default.rules`。
//!
//! # 设计要点
//! - 使用 [`arc_swap::ArcSwap`] 实现策略的热更新,无需重启即可生效。
//! - 通过 [`spawn_blocking`] 在阻塞线程中执行解析,避免阻塞 tokio runtime。
//! - 内置 [`BANNED_PREFIX_SUGGESTIONS`] 列表,用于在审批提示中给出更安全的替代命令建议。
...
/// 策略要求审批,但 `AskForApproval` 设为 `Never` 时的冲突原因描述。
const PROMPT_CONFLICT_REASON: &str =
    "approval required by policy, but AskForApproval is set to Never";
/// 策略要求 sandbox 审批,但 `AskForApproval::Granular.sandbox_approval` 为 false 时的拒绝原因。
...
/// 策略规则要求审批,但 `AskForApproval::Granular.rules` 为 false 时的拒绝原因。
...
/// 当某条命令前缀被规则禁止时,用于在审批提示中给出"使用更具体的入口"建议的前缀列表。
///
/// 例如,当 `bash` 被禁时,会建议改用 `bash -lc <具体命令>` 形式以让规则可以精确评估。
