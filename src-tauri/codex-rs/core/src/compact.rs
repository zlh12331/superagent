...
//!
//! 当 turn 的对话历史接近模型 context window 上限时,通过 compact 操作将历史
//! 折叠为摘要(summary),从而释放上下文空间供后续 turn 使用。
//!
//! # 压缩时机
//! - **Pre-turn / 手动压缩**:在 turn 开始前用摘要替换历史,并清空 `reference_context_item`,
//!   下一个常规 turn 会重新注入完整 initial context。
//! - **Mid-turn 压缩**:在 turn 进行中触发,需在最后一条真实 user message 之前注入
//!   initial context,因为模型被训练为期望压缩摘要是历史中的最后一项。
//!
//! # 与其他模块的关系
//! 该模块依赖 [`Session`] 构建压缩所需的 input,通过 [`ModelClientSession`] 调用
//! `/responses/compact` 端点完成摘要生成,并触发 pre/post compact hook。
...
/// 控制压缩后替换历史是否需要包含 initial context。
///
/// # 变体说明
/// - `DoNotInject`:用于 pre-turn / 手动压缩。会用摘要替换历史并清空 `reference_context_item`,
///   下一个常规 turn 会在压缩后完整重新注入 initial context。
/// - `BeforeLastUserMessage`:用于 mid-turn 压缩。模型被训练为期望压缩摘要是
///   压缩后历史中的最后一项,因此必须在最后一条真实 user message 之前注入 initial context。
#[derive(Debug)]
pub(crate) enum InitialContextInjection {
    /// 在最后一条 user message 之前注入指定 world state 渲染出的 initial context。
    BeforeLastUserMessage(Arc<WorldState>),
    /// 不注入 initial context,压缩后历史仅保留摘要。
    DoNotInject,
}

/// 构建压缩所需的 initial context,并返回渲染后的 items 与对应的 world state。
///
/// 返回值中的 items 将作为压缩后历史的前置上下文,world state 用于保持 history baseline 一致。
pub(crate) async fn build_compaction_initial_context(
    sess: &Session,
    turn_context: &TurnContext,
    initial_context_injection: &InitialContextInjection,
) -> (Vec<ResponseItem>, Option<Arc<WorldState>>) {
    // 返回渲染后的 state 及其 items,使 history 与其 baseline 保持一致。
