//! agent 角色(role)模块。
//!
//! 在已有 session config 基础上叠加 agent-role 配置层。
//!
//! # 设计概要
//! role 在 spawn 时选定,并通过与 `config.toml` 相同的 config 加载机制载入。
//! 本模块负责:
//! - 解析内置与用户自定义 role 文件;
//! - 将 role 作为高优先级 config layer 插入;
//! - 除非 role 显式设置,否则保留 caller 当前的 provider 与 service_tier。
//!
//! # 职责边界
//! 本模块不负责"何时 spawn sub-agent"或"使用哪个 role",
//! 该编排由 multi-agent tool handler 负责。
...
/// caller 未指定 `agent_type` 时使用的默认 role 名。
pub const DEFAULT_ROLE_NAME: &str = "default";
const AGENT_TYPE_UNAVAILABLE_ERROR: &str = "agent type is currently not available";

/// 将具名 role 层应用到 `config`,同时保留 caller 持有的 provider 设置。
///
/// # 优先级
/// role 层以 session-flag 优先级插入,使其能够覆盖已持久化的 config;
/// 但 caller 当前的 `model_provider` 与 `service_tier` 仍是粘性的运行时选择,
/// 除非 role 显式设置对应的顶层 config key。
///
/// # 设计原因
/// 若在不保留 override 的情况下重建 config,spawn 出的 agent 会静默回退到默认设置,
/// 这通常不是调用方期望的行为。
