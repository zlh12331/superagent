//! agent 注册表(registry)模块。
//!
//! 对 Codex 的多 agent 能力施加限制。当前实现限制:
//! - 每个 user session 内 sub-agent(即 thread)的总数。
//!
//! 该结构由同一 user session 中的所有 agent 共享(因为 [`crate::agent::control::AgentControl`] 是共享的)。
...
/// agent 注册表,用于追踪 user session 内所有 active agent 并施加限制。
///
/// # 共享语义
/// 该结构由同一 user session 中的所有 agent 共享,
/// 因此使用 `Mutex` / `AtomicUsize` 保护内部状态。
#[derive(Default)]
...
/// 单个 agent 的元数据。
///
/// 由注册表维护,用于在多 agent 编排中标识与定位 agent。
...
/// 格式化 agent 昵称,在重置时附加序数后缀(如 "1st"、"2nd"、"3rd")。
...
/// 计算给定 session source 的 spawn 深度。
///
/// 仅 `SubAgent::ThreadSpawn` 携带 depth 信息,其他来源视为深度 0。
