//! agent 控制模块。
//!
//! 提供 [`AgentControl`],作为多 agent 场景下 spawn / 等待 / 关闭子代理的统一入口。
//!
//! # 主要职责
//! - 维护 agent 注册表([`AgentRegistry`])与执行限制器([`AgentExecutionLimiter`]);
//! - 根据 fork mode 决定子代理初始 history(truncate 或 full);
//! - 协调 subagent 与 root thread 之间的 inter-agent 消息通信;
//! - 处理 subagent 的 residency(v2 多 agent 版本下子代理驻留策略)。
...
/// root thread 在 inter-agent 任务列表中的标识名称。
