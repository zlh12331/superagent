//! Realtime 对话管理。
//!
//! 本模块负责管理与 OpenAI Realtime API 的 WebSocket/WebRTC 会话生命周期，
//! 包括：会话启动/关闭、音频帧与文本的输入输出、handoff（后台 agent 委派）的双向流转、
//! 以及 realtime 事件向 Codex 事件流的扇出（fanout）。
//!
//! 支持 V1 与 V2 两种 realtime 协议版本，通过 `RealtimeSessionKind` 区分行为差异。
...
/// realtime startup context 的 token 预算。
const REALTIME_STARTUP_CONTEXT_TOKEN_BUDGET: usize = 5_300;
/// 后端 agent 输出文本的 token 预算。
...
/// V2 会话 handoff 转向（steer）时发送给 realtime 的确认文本。
const REALTIME_V2_STEER_ACKNOWLEDGEMENT: &str =
    "This was sent to steer the previous background agent task.";
/// realtime API 返回"已有活跃响应"错误时的消息前缀，用于判断是否需要延迟重试。
...
/// Realtime 对话管理器，持有当前活跃会话的状态。
///
/// 通过 `Mutex<Option<ConversationState>>` 保证同一时刻仅有一个活跃会话；
/// 启动新会话前会先停止旧会话。
...
/// 指示是否应使用配置文件中的 voice 设置。
///
/// WebRTC 无显式版本时忽略配置 voice（由 AVAS 后端决定），其他场景使用配置值。
#[derive(Clone, Copy)]
...
/// 构建 realtime 会话配置，合并用户参数、配置文件默认值与 startup context。
///
/// 处理 instructions（prompt + startup context）、model、voice、output modality 等，
/// 并根据版本号选择对应的 event parser。
...
/// 处理 realtime 音频帧输入，将其送入活跃会话的音频队列。
...
fn realtime_api_key(auth: Option<&CodexAuth>, provider: &ModelProviderInfo) -> CodexResult<String> {
    ...
    // TODO(aibrahim): 待 realtime 鉴权不再要求 ChatGPT/SIWC 会话使用 API key 后，
    // 移除此临时回退逻辑。
...
/// 处理 realtime 文本输入，将其送入活跃会话的文本队列。
...
/// 处理 realtime 语音合成请求，将文本作为 speech 追加到活跃会话。
...
async fn run_realtime_input_task(input: RealtimeInputTask) {
    ...
    loop {
        let result = tokio::select! {
            ...
            // 后台 agent 进度或最终输出，需回传给 realtime。
    ...
    let result = match event_parser {
        RealtimeEventParser::V1 => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text } => {
                // TODO(guinness): 待 API 变更完成后，改用新的独立 handoff 客户端事件。
                writer
