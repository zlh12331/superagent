//! 与模型 provider API 通信的会话级与 turn 级辅助封装。
//!
//! # 设计概览
//! [`ModelClient`] 的生命周期与一个 Codex 会话(thread)一致,持有调用 provider 所需的稳定配置与状态:
//! 认证信息、provider 选择、conversation id 以及传输层回退(fallback)状态。
//!
//! # turn 级参数显式传递
//! turn 级设置(模型选择、reasoning 控制、telemetry 上下文、turn 元数据)
//! 通过参数显式传递给流式与 unary 方法,使 turn 的生命周期在调用处一目了然。
//!
//! # WebSocket 会话与 prewarm
//! [`ModelClientSession`] 在每个 turn 创建,用于在该 turn 内流式发起一个或多个 Responses API 请求。
//! 它会缓存一个 Responses WebSocket 连接(惰性打开),并保存 turn 级状态,
//! 例如用于粘性路由(sticky routing)的 `x-codex-turn-state` token。
//!
//! WebSocket prewarm 是 v2 专用的 `response.create`(设置 `generate=false`),
//! 它会等待完成,以便下一次请求可以复用同一连接与 `previous_response_id`。
//!
//! turn 执行时,在首次流式请求之前会尽力(best-effort)执行 prewarm,
//! 以便后续请求能够复用同一连接。
//!
//! # 重试预算的权衡(Retry-Budget Tradeoff)
//!
//! WebSocket prewarm 被视为一个 turn 内的首次 WebSocket 连接尝试。
//! 若 prewarm 失败,正常的流式重试/回退逻辑会在同一 turn 内完成恢复。
...
// `/responses/compact` 是 unary 调用,因此该超时覆盖整个响应,而非流式事件之间的空闲间隔。
...
/// `compact` 会话(上下文压缩)的请求级配置。
///
/// 这些字段对应一次 `/responses/compact` 调用所需的 reasoning 强度、摘要展示方式
/// 与 service_tier,均由调用方按 turn 显式提供。
...
/// 会话级共享状态,被所有 [`ModelClient`] 克隆共享。
///
/// 这里只保留最小集合,以便 `ModelClient` 不必持有完整 `Config`。
/// 大部分配置是 turn 级的,通过参数显式传递给流式 / unary 方法。
#[derive(Debug)]
...
/// 单次请求尝试所使用的 API client 解析结果。
///
/// 将认证、provider、agent 身份等字段捆绑在一起,
/// 确保 prewarm 路径与正常请求路径共享同一套 auth/provider 解析流程,
/// 避免出现两套不一致的初始化逻辑。
...
/// 面向模型 provider API 的会话级客户端。
///
/// 持有跨 turn 共享的配置与状态(认证、provider 选择、thread id、传输层回退状态)。
///
/// # WebSocket 回退的会话级语义
/// 一旦某个 turn 启用了 HTTP 回退,本会话剩余的所有 turn 都会继续使用 HTTP。
///
/// # turn 级参数显式传递
/// turn 级设置(模型选择、reasoning 控制、telemetry 上下文、turn 元数据)
/// 通过参数显式传递给相关方法,使 turn 生命周期在调用处清晰可见。
#[derive(Debug, Clone)]
...
/// 由 [`ModelClient`] 派生的 turn 级流式会话。
///
/// 该会话惰性建立 Responses WebSocket 连接,并在 turn 内的多次请求间复用。
/// 它还缓存以下 turn 级状态:
///
/// - 上一次完整请求:仅当当前请求是上一次请求的增量扩展时,
///   后续调用才能复用增量 websocket 请求 payload。
/// - `x-codex-turn-state` 粘性路由 token:必须在本 turn 内的所有请求中重复发送。
///
/// # 重要约束
/// 每个 Codex turn 都必须创建新的 `ModelClientSession`。跨 turn 复用会把上一个 turn 的
/// 粘性路由 token 带入新 turn,违反客户端 / 服务端契约,可能引发路由 bug。
pub struct ModelClientSession {
    client: ModelClient,
    websocket_session: WebsocketSession,
    /// 用于粘性路由(sticky routing)的 turn 状态。
    ///
    /// 这是一个 `OnceLock`,在 turn 开始时通过 `x-codex-turn-state` 响应头从服务端接收并存储。
    /// 一旦设置,本 turn 内所有后续请求都应在 `x-codex-turn-state` 请求头中回传该值,
    /// 以维持粘性路由。
    ///
    /// 这是客户端与服务端之间的契约:turn 开始时接收、turn 内多次请求之间不变地重复发送
    /// (例如重试、增量追加、续传请求),并且禁止跨 turn 发送。
...
/// Responses WebSocket 连接的会话级缓存。
///
/// 保存连接本身、上次完整请求、上次响应接收端,
/// 以及用于追踪连接复用与 warmup 来源的标志位。
...
// 这里刻意不实现为 `PartialEq`:请求相等性需要比较 `input` 与 `client_metadata`,
// 而 websocket 复用判断需要单独比较 input 并忽略 metadata。
// 保持解构为全字段匹配,这样将来新增字段时,必须显式决定复用策略。
