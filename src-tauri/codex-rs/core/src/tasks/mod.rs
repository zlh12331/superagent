//! Task 模块:turn 内异步任务的生命周期管理。
//!
//! 定义 [`RegularTask`]、[`CompactTask`]、[`UserShellCommandTask`] 等
//! 任务类型,以及任务的 spawn、取消、超时等通用逻辑。
//!
//! # 子模块
//! - [`lifecycle`]:任务生命周期(spawn、cancel、abort、超时);
//! - [`regular`]:常规任务(模型推理);
//! - [`compact`]:context 压缩任务;
//! - [`review`]:review thread 任务;
//! - [`user_shell`]:用户 shell 命令任务。
