//! 配置(config)模块。
//!
//! Codex core 的配置入口,整合来自多层的配置来源:
//! - 内置默认值
//! - 用户全局 `~/.codex/config.toml`
//! - 项目级 `<project>/AGENTS.md` / `.codex/config.toml`
//! - 运行时 overrides(命令行、session flag、role 层)
//!
//! # 主要职责
//! - 加载与合并 config layer(基于 [`codex_config::ConfigLayerStack`]);
//! - 暴露 [`Config`] 作为最终生效配置的不可变快照;
//! - 提供 [`ConfigEditsBuilder`] 用于构造配置编辑操作;
//! - 校验配置约束(模型 provider、permission profile、sandbox 模式等)。
//!
//! # 与其他模块的关系
//! 几乎所有模块都通过 [`Config`] 读取运行参数;
//! [`crate::agent::role`] 通过本模块叠加 role 层;
//! [`crate::exec_policy`] 通过本模块定位 rules 目录。
