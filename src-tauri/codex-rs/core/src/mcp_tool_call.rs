//! MCP tool call 执行模块。
//!
//! 负责处理模型发起的 MCP 工具调用完整流程,包括:
//! - 参数解析与 OpenAI file 引用重写;
//! - approval 流程(支持 guardian review、hooks、人工确认);
//! - 沙箱权限请求;
//! - 实际调用 MCP server 并收集结果;
//! - app tool policy 评估;
//! - telemetry / analytics 上报。
//!
//! # 与其他模块的关系
//! - 由 session turn 循环在收到 `function_call` item 时调用;
//! - 依赖 [`crate::guardian`] 进行审批;
//! - 依赖 [`crate::mcp`] 进行实际 MCP 调用;
//! - 通过 [`crate::hook_runtime`] 触发权限请求 hooks。
