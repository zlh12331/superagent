//! 实时（realtime）会话启动上下文构建。
//!
//! 本模块在 realtime 会话启动时，从当前线程历史、本地线程元数据与受限的工作区扫描中
//! 拼装一段 startup context，作为背景信息注入到 realtime 模型的 instructions 中。
//! 内容包括：当前线程最近几轮、按 git 仓库分组的近期会话、工作区目录树概览。
...
/// startup context 的头部说明文本，提示模型将其作为背景信息而非重复内容。
...
/// 构建 realtime 会话的 startup context 文本。
///
/// 汇总当前线程最近几轮、按 git 仓库分组的近期会话、工作区目录树概览，
/// 各分区按独立 token 预算截断后拼接，最终用 `<startup_context>` 标签包裹。
/// 若所有分区均无内容则返回 `None`。
...
/// 将文本截断到指定的 token 预算内。
///
/// 由于共享的截断器在选择保留内容后会追加标记文本，因此需要逐步收紧内容预算，
/// 直到渲染后的文本（含标记）满足 per-turn 上限。
pub(crate) fn truncate_realtime_text_to_token_budget(text: &str, budget_tokens: usize) -> String {
    ...
    loop {
        ...
        // 共享的截断器在选择保留内容后会追加标记，
        // 因此需要逐步收紧内容预算，直到渲染后的 turn 本身满足 per-turn 上限。
