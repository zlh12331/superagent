//! turn 元数据模块。
//!
//! 维护一次 turn(模型推理轮次)生命周期内需要随请求携带的元数据,
//! 包括 workspace git 信息、sandbox 标签、subagent 标识、reasoning 设置等。
//!
//! # 架构位置
//! 该模块位于 session 层与 client 层之间:session 在 turn 启动时构建 [`TurnMetadataState`],
//! client 在发起 Responses API 请求时读取该状态以填充请求头与 client_metadata。
...
/// turn 级 MCP 元数据上下文,用于在 MCP 调用过程中传递 turn 信息。
pub(crate) struct McpTurnMetadataContext<'a> {
    pub(crate) model: &'a str,
    pub(crate) reasoning_effort: Option<ReasoningEffortConfig>,
}

/// workspace 级 git 元数据快照。
///
/// 记录当前 workspace 关联的远端 URL、最新 commit hash 以及是否有未提交改动,
/// 用于在 turn 元数据中携带上下文,辅助服务端分析。
...
/// 构造 detached memory 请求所需的 Responses API 元数据。
///
/// detached memory 是不依赖 thread 上下文的独立 memory 操作,
/// 该函数将 installation、session、thread、window 等基础信息与
/// memory 专属的 workspace 信息组装为 [`CodexResponsesMetadata`]。
...
/// turn 元数据的可变状态。
///
/// 该结构在 turn 启动时构建,字段以 `Arc<RwLock<...>>` / `Arc<Mutex<...>>` 形式共享,
/// 以便在 turn 进行过程中多个任务(如 git 探测任务、telemetry 任务)能够并发更新。
#[derive(Clone, Debug)]
