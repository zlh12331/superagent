//! 工具生命周期通知实现。
//!
//! 本文件提供向所有扩展的工具生命周期 contributor 发送工具启动/完成/中止事件的辅助函数。
...
/// 通知所有 contributor 工具调用完成（包含具体 outcome）。
...
/// 通知所有 contributor 工具调用被中止（outcome 固定为 `Aborted`）。
...
/// 通知 contributor 工具调用完成的内部实现，按字段拆分以便复用。
...
/// 将 core 端的 `ToolCallSource` 转换为扩展 API 的 `ToolCallSource`。
