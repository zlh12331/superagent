//! Tools 模块:codex 工具系统。
//!
//! 定义工具的注册、路由、并行执行、spec 解析、事件分发等核心机制,
//! 以及各类具体工具(shell、unified exec、apply patch、MCP、
//! multi-agent、plan、view image 等)的实现。
//!
//! # 主要子模块
//! - [`registry`]:工具注册表;
//! - [`router`]:工具路由;
//! - [`parallel`]:并行工具执行;
//! - [`lifecycle`]:工具调用生命周期;
//! - [`spec_plan`]:spec 与 plan 解析;
//! - [`handlers`]:各工具的具体处理器;
//! - [`runtimes`]:工具运行时(shell、apply patch、unified exec);
