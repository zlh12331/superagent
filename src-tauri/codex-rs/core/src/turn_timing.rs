//! turn 计时模块。
//!
//! 负责测量一次 turn 内的关键时延指标,用于性能分析与遥测:
//!
//! - **TTFT**(Time To First Token):从 turn 开始到首个 token 输出的延迟。
//! - **TTFM**(Time To First Message):从 turn 开始到首条 AgentMessage 输出的延迟。
//! - **TurnProfile**:turn 内各阶段(首次采样前、采样中、采样间空闲、tool 阻塞、末次采样后)的耗时分布。
//!
//! # 设计要点
//! 所有计时状态以 `Arc<...>` 形式在 turn 内共享,允许多个任务并发更新。
//! `TurnProfileTimingGuard` 基于 RAII,在 drop 时自动结束当前阶段,简化调用方代码。
...
/// 记录 turn 的 TTFT(time to first token)指标。
///
/// 仅在首次记录时返回有效的 [`Duration`],并写入 session telemetry。
...
/// 记录 turn 的 TTFM(time to first message)指标。
///
/// 仅在首个 [`TurnItem::AgentMessage`] 出现时返回有效 [`Duration`],
/// 并通过 otel 记录到 `TURN_TTFM_DURATION_METRIC`。
...
///
/// 内部由两部分组成:
/// - `state`:TTFT / TTFM 等关键时延的记录状态,使用 tokio `Mutex` 保护;
/// - `profile`:turn 内各阶段耗时的剖析状态,使用 std `Mutex` 保护(因为采样操作短促且无 await)。
...
/// turn 性能剖析的内部状态。
///
/// 通过 `advance` 在阶段切换时累加上一阶段的耗时,
/// 最终在 `complete` 中产出 [`TurnProfile`]。
#[derive(Debug, Default)]
...
/// RAII 风格的阶段计时守卫。
///
/// 通过 [`TurnTimingState::begin_sampling`] / [`TurnTimingState::begin_tool_blocking`] 创建,
/// drop 时自动调用 `end_phase` 结束当前阶段,确保阶段切换不会因为早期 return 而遗漏。
#[must_use]
