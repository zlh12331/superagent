...
/// thread(会话)配置的不可变快照。
///
/// 在 thread 启动或配置变更时由 [`CodexThread`] 产生,
/// 用于向上层(UI / app-server)暴露 thread 当前的全部配置信息。
/// 字段映射了 thread 启动时生效的模型、provider、审批策略、permission profile、
/// 环境、workspace、reasoning 设置等。
#[derive(Clone, Debug)]
...
/// 解释 `CodexThread::try_start_turn_if_idle` 为何拒绝一次自动 idle turn。
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum TryStartTurnIfIdleRejectionReason {
    /// 已有用户 / 客户端触发的 mailbox 工作排队,
    /// 必须优先于 extension 发起的 idle 工作。
    PendingTriggerTurn,
    /// 当前 thread 处于 Plan 模式,自动 idle 工作不应在该模式下
    /// 启动新的模型 turn。
    PlanMode,
    /// 已有其他 turn 或 task 进行中,或在自动 turn 启动前 idle 预留已被释放。
    Busy,
}

/// 当 extension 请求启动自动 idle 工作但 thread 不满足执行条件时返回的拒绝信息。
#[derive(Debug)]
...
impl TryStartTurnIfIdleError {
    ...
    /// 返回自动 idle turn 被拒绝的稳定原因,便于上层据此决定后续策略。
    pub fn reason(&self) -> TryStartTurnIfIdleRejectionReason {
        self.reason
    }

    /// 消费拒绝错误并原样返回最初的模型可见 input,
    /// 以便调用方显式地重试、丢弃或记录该 input。
...
/// app-server 在启动 turn 前需要校验的 thread 设置覆盖项。
///
/// 每个字段为 `Option`,仅当对应字段需要被显式覆盖时才设值,
/// 未设值的字段将沿用 thread 当前配置。
...
/// Codex thread(会话)句柄,作为构成一个 thread 的双向消息流的对外入口。
///
/// 持有底层的 [`Codex`] session、session 配置事件、rollout 文件路径等信息,
/// 是上层(TUI / app-server / CLI)与 codex-core 交互的主要对象。
...
/// 后台终端(已分离的 shell 进程)的描述信息。
///
/// 用于追踪在 thread 之外仍持续运行的后台终端进程,便于后续重新关联或查询状态。
...
/// 构成 thread(早期称为 conversation)的双向消息流的对外入口与 conduit。
impl CodexThread {
    pub(crate) fn new(
        codex: Codex,
        session_configured: SessionConfiguredEvent,
    ...
    /// 提交一个 [`Op`] 到 thread 的 mailbox。
    ///
    /// 返回值为该 op 分配的 submission id。
    pub async fn submit(&self, op: Op) -> CodexResult<String> {
        self.codex.submit(op).await
    }

    /// 返回用于 thread 级生产环境遥测的 session telemetry handle。
    pub fn session_telemetry(&self) -> SessionTelemetry {
        self.codex.session.services.session_telemetry.clone()
    }

    /// 关闭 thread 并等待底层 session loop 完全退出。
    pub async fn shutdown_and_wait(&self) -> CodexResult<()> {
        self.codex.shutdown_and_wait().await
    }

    /// 等待底层 session loop 终止。
    ...
    /// 提交一个 [`Op`] 并附带可选的 W3C trace context,
    /// 用于跨服务分布式追踪。
