//! JSON Schema 生成模块。
//!
//! 该模块为 memories 工具的输入/输出类型生成 JSON Schema，
//! 供 Responses API 工具定义使用。生成的 schema 采用 Draft 2019-09 规范，
//! 并将所有子 schema 内联（`inline_subschemas = true`）以避免引用依赖。
...
/// 为类型 `T` 生成工具输入 schema。
///
/// 输入 schema 不为 `Option` 字段添加 null 类型，
/// 以确保模型生成的参数严格匹配类型定义。
...
/// 为类型 `T` 生成工具输出 schema。
///
/// 输出 schema 为 `Option` 字段添加 null 类型，
/// 以兼容 JSON 序列化时 `None` 值的表示。
...
/// （`properties`、`required`、`type`、`additionalProperties`、`$defs`、`definitions`）。
fn schema_for<T: JsonSchema>(option_add_null_type: bool) -> Value {
    ...
    // 仅提取工具定义需要的字段，丢弃 schema 元数据（如 $schema、title 等）
