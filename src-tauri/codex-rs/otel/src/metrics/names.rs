//! Metrics 指标名称常量定义。
//!
//! 集中声明所有 Codex 使用的 metrics 指标名称，确保名称一致性。
//! 名称采用 `codex.<domain>.<metric>` 的点分命名规范。
