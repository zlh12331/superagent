//! 持久化 Codex 会话 rollout（.jsonl），使会话可在后续重放或检查。
...
/// 将规范的会话 rollout item 写入 JSONL 文件。
///
/// Rollout 以 JSONL 格式记录，可使用如下工具检查：
///
/// ```ignore
...
/// 后台 rollout writer 持有的可变状态。
///
/// Items 先被追加到 `pending_items`；persist/flush/shutdown 仅在某条 item 成功写入后
/// 才将其从该队列移除。IO 失败会丢弃文件句柄但保留未写入的后缀，
/// 以便下一次屏障操作能重新打开文件并重试。
...
/// 将一个已过滤的 rollout item 追加到现有 rollout JSONL 文件。
///
/// 该函数用于对未加载 thread 的元数据更新。活跃会话应使用
/// `RolloutRecorder::record_canonical_items`，以保持 rollout 写入与会话流的顺序一致。
///
/// # 参数
/// - `path`: rollout 文件路径
/// - `item`: 待追加的 rollout item
///
/// # 返回值
/// 返回 `Ok(())` 表示追加成功；发生 IO 错误则返回 `Err`。
