...
/// 从 rollout item 列表构造线程元数据构造器。
///
/// 优先从 `SessionMeta` 条目中提取元数据；若不存在则回退到从文件名解析时间戳与 ID。
///
/// # 参数
...
///
/// # 返回值
/// 返回 [`ThreadMetadataBuilder`]；若无法解析出元数据则返回 `None`。
...
///
/// # 返回值
/// 返回提取结果 [`ExtractionOutcome`]；若文件为空或解析失败则返回 `Err`。
