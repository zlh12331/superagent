...
/// 搜索匹配结果，以每个 rollout 的规范 `.jsonl` 路径为键。
pub type RolloutSearchMatches = HashMap<PathBuf, Option<String>>;

/// 搜索 rollout 文件路径集合，返回匹配的路径集合。
///
/// # 参数
/// - `rg_command`: ripgrep 可执行文件路径
/// - `codex_home`: Codex 主目录
/// - `archived`: 是否搜索归档目录
/// - `search_term`: 搜索词
///
/// # 返回值
/// 返回匹配的 rollout 路径集合；若发生 IO 错误则返回 `Err`。
...
/// 搜索 rollout 文件并返回每个匹配文件对应的搜索片段（snippet）。
///
/// 优先使用 ripgrep 搜索 plain `.jsonl` 文件，再扫描压缩文件。
///
/// # 参数
/// - `rg_command`: ripgrep 可执行文件路径
/// - `codex_home`: Codex 主目录
...
///
/// # 返回值
/// 返回 [`RolloutSearchMatches`]；若发生 IO 错误则返回 `Err`。
...
/// 返回 rollout 文件中第一条匹配搜索词的内容片段（snippet）。
///
/// # 参数
/// - `path`: rollout 文件路径
/// - `search_term`: 搜索词
///
/// # 返回值
/// 返回匹配片段；若无匹配则返回 `Ok(None)`；若发生 IO 错误则返回 `Err`。
