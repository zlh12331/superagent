...
// 函数门禁（工程化强制）：形参数 ≤4 + 函数体 ≤100 净行，双指标棘轮 + 体长分档
// ──────────────────────────────────────────────────────────────
// 依据 typescript-dev-standards-ai.md §函数（参数≤4 对象封装 / 函数体≤40 行）
// 与业界实践（eslint max-params 3-4、max-lines-per-function 常见 50）。
//
// 2026-09-10 门槛调整（用户拍板）：棘轮超限门槛由 40 放宽至 100——
// 「≤100 行不进棘轮、不入基线、不判违规」；100 行以下的函数体不再被门禁管理，
// 降低 41–100 行轻量函数（IPC mock 表 / React JSX 树等声明平铺型）的噪音。
// 40 行仍作为规范目标在输出中展示（见「距规范目标」），>100 行起进棘轮。
...
//   轻度 100–200 / 中度 200–400 / 重债 >400 必须登记理由。
//   分档只影响输出分级与 >400 档的登记校验，不改变棘轮判定。
//   为什么分档：超限里大量集中在 100–200 行的中度欠债，混在一个数字里看不出
